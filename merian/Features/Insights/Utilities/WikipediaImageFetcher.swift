...
@MainActor
final class WikipediaImageFetcher: ObservableObject {
    ...
    func fetchImage(for scientificName: String) async {
        ...
        do {
            ...
            let (imageData, imageResponse) = try await URLSession.shared.data(for: URLRequest(url: imageUrl))
            ...
            MerianLog.general.debug("WikipediaImageFetcher failed for \(scientificName): \(error.localizedDescription)")
        }
    }
}
