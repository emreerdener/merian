...
Navigate to `apps/ios/Merian/Core/Hardware/CameraManager.swift`. We need to intercept the `capturePhoto()` output and artificially substitute the memory buffer with our mock payload before the image lands in the `OfflineQueueManager`.
