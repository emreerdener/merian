...
struct MerianNetworkClientTests {
    ...
    @Test func testSpeciesDictionaryFieldChatUsesStrictSpeciesContract() async throws {
        ...
        MockURLProtocol.mockEndpoints["/species-dictionary-chat"] = { request in
            #expect(request.httpMethod == "POST")
            let body = try #require(MockURLProtocol.bodyData(for: request))
            let payload = try #require(
                JSONSerialization.jsonObject(with: body) as? [String: Any]
        ...
        let loaded = try await MerianNetworkClient.shared
            .loadSpeciesDictionaryChat(speciesId: speciesID)
        #expect(loaded.subjectId == speciesID)

        let sent = try await MerianNetworkClient.shared
    ...
    @Test func testFieldChatRejectsMalformedOrCrossSubjectSuccessResponses() async throws {
        ...
        MockURLProtocol.mockEndpoints["/species-dictionary-chat"] = { _ in
            (response, chatResponse(subjectID: otherSubjectID))
        }
        await #expect(throws: MerianError.invalidResponse) {
            try await MerianNetworkClient.shared
