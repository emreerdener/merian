...
@Suite("Notification and Public Profile Endpoint Transport")
@MainActor
struct NotificationAndPublicProfileEndpointTransportTests {
    @Test func requestInventoryRetainsReadAndMutationBoundaries() {
        let operations = NotificationAndPublicProfileEndpointRequestCase.operations
        #expect(operations.count == 8 && Set(operations.map(\.function)).count == 8)
        #expect(NotificationAndPublicProfileEndpointRequestCase.typedOperations.count == 7)
        #expect(NotificationAndPublicProfileEndpointRequestCase.mutations.count == 5)
        #expect(Set(NotificationAndPublicProfileEndpointRequestCase.readOperations.map(\.function)) == [
            "get-explore-notifications", "get-explore-unread-notification-count", "check-public-username"
        ])
        #expect(Set(NotificationAndPublicProfileEndpointRequestCase.mutations.map(\.function)) == [
            "mark-explore-notifications-read", "register-push-device",
            "update-public-username", "update-public-display-name", "update-public-avatar"
        ])
    }

    @Test(arguments: NotificationAndPublicProfileEndpointRequestCase.typedOperations, ["", "not-json", "{}", #"{"success":true,"data":null}"#])
    func malformedTypedSuccessStillThrowsDecodingError(
        _ testCase: NotificationAndPublicProfileEndpointRequestCase,
        responseJSON: String
    ) async {
    ...
    @Test(arguments: NotificationAndPublicProfileEndpointRequestCase.operations, [400, 401, 404, 409])
    ...
    @Test(arguments: NotificationAndPublicProfileEndpointRequestCase.operations, [200, 401, 503])
    ...
    @Test(arguments: NotificationAndPublicProfileEndpointRequestCase.readOperations, [true, false])
    ...
    @Test(arguments: NotificationAndPublicProfileEndpointRequestCase.operations)
    func cancellationBeforeDispatchDoesNotSend(_ testCase: NotificationAndPublicProfileEndpointRequestCase) async {
        let fixture = NetworkEndpointFixture()
        defer { fixture.close() }

        await confirmation("No cancelled notification or public-profile POST", expectedCount: 0) { sent in
            fixture.transport.register(path: testCase.path) { request in
                sent()
                return try NetworkEndpointTestSupport.response(to: request, json: testCase.responseJSON)
            }
    ...
    @Test func pushRegistrationIgnoresSuccessfulBodies() async throws {
        let testCase = NotificationAndPublicProfileEndpointRequestCase.registerPush
    ...
    @Test(arguments: NotificationAndPublicProfileEndpointRequestCase.mutations, [false, true])
