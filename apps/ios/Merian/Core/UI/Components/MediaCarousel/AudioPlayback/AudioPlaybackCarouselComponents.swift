...
struct AudioPlaybackCarouselContent: View {
    ...
    let audioBoostPillState: AudioBoostPillState?
        ...
        ZStack {
            ...
            if isDecoding {
                ...
                if showsBoostPreparationStatus {
                    AudioPlaybackStatusMessage(text: "Boosting audio…")
                }

                if showsBoostFailure {
                    AudioPlaybackStatusMessage(
...
private struct AudioPlaybackPrimaryControl: View {
    let isPlaying: Bool
    let isDisabled: Bool
...
private struct AudioPlaybackBadges: View {
    let pillState: AudioBoostPillState?
    let elapsedText: String?
    let durationText: String?
    let onToggleBoost: () -> Void
...
private struct AudioPlaybackStatusMessage: View {
    let text: String
...
private extension View {
    func audioPlaybackBadgeStyle() -> some View {
        self
            .font(.caption.weight(.semibold))
            .foregroundStyle(.white)
            .lineLimit(1)
