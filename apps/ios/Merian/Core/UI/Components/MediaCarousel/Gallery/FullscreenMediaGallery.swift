...
struct FullscreenMediaGallery: View {
    let presentation: MediaGalleryPresentation
    ...
    init(
        presentation: MediaGalleryPresentation,
    ...
    private var selectedItem: MediaGalleryItem? {
                   ...
                   !isSelectedVideoUnavailable {
                    Button {
                        ...
                        dependencies.selectionFeedback(
                            dependencies.feedbackIdentifier(
                                isVideoMuted
                                    ? .fullscreenMuteOn
                                    : .fullscreenMuteOff
                            )
    ...
    @ViewBuilder
    private func galleryContent(for item: MediaGalleryItem) -> some View {
    ...
    @ViewBuilder
    private func galleryMedia(for item: MediaGalleryItem) -> some View {
        switch item.source {
        case .liveImage(let data):
            FullscreenMediaLiveImageView(data: data)
        ...
        case .videoPath(let path):
            FullscreenMediaVideoPlaybackView(
                path: path,
