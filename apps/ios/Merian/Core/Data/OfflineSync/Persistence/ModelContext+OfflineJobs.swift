...
extension ModelContext {
    ...
    ///
    /// Re-queue attempts are idempotent so callers can safely delete a record
    /// when an earlier sync pass already staged its cloud erasure.
    @discardableResult
    func ensurePendingCloudDeletionTask(
        ...
        let task = PendingCloudDeletionTask(
            scanId: scanId,
            timestamp: timestamp
        )
        insert(task)
