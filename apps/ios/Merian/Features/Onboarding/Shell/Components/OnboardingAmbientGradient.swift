...
struct OnboardingAmbientGradient: View {
    // MARK: - Motion Dependencies
    @Environment(\.accessibilityReduceMotion) private var reduceMotion
    @State private var isShifted = false

    private let isHardwareAnimationEnabled: @MainActor () -> Bool

    init(
        isHardwareAnimationEnabled: @escaping @MainActor () -> Bool
    ) {
        self.isHardwareAnimationEnabled = isHardwareAnimationEnabled
    }

    private var shouldAnimate: Bool {
        isHardwareAnimationEnabled() && !reduceMotion
    }
