...
//     Optional so lightweight migration adds the column with NULL; the accessor
//     defaults to "unreviewed" at runtime.
//
// MARK: - Frozen snapshot for MerianSchemaV36
// LocalScanRecord changed in V36 — it is frozen here as a namespaced inner class.
// OfflineQueuedScan, PendingCloudDeletionTask, and UserSpeciesPreference did NOT change in V36;
// they are aliased to prior frozen snapshots so the V36 checksum stays fixed even when
// later versions add new fields to the global active types.
...
enum MerianSchemaV36: VersionedSchema {
    ...
    static var models: [any PersistentModel.Type] {
        [MerianSchemaV36.LocalScanRecord.self, MerianSchemaV36.OfflineQueuedScan.self,
         MerianSchemaV36.ScanCollection.self, MerianSchemaV36.PendingCloudDeletionTask.self,
         MerianSchemaV36.UserSpeciesPreference.self]
    }

    typealias OfflineQueuedScan         = MerianSchemaV35.OfflineQueuedScan
    typealias PendingCloudDeletionTask  = MerianSchemaV30.PendingCloudDeletionTask
    typealias UserSpeciesPreference     = MerianSchemaV35.UserSpeciesPreference

    // ScanCollection redeclared so its inverse relationship points to V36.LocalScanRecord.
    @Model
    ...
    // MARK: - Frozen LocalScanRecord snapshot for MerianSchemaV36
    // Captures the LocalScanRecord shape AT V36 — with userReviewStateRaw, without observationContextJSON.
    // DO NOT modify after committing. Checksums must remain stable forever.
    @Model
    final class LocalScanRecord {
        ...
        @Attribute var userReviewStateRaw: String?
