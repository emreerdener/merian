...
// Frozen from the processed V50 release graph after the Swift-side collection
// tombstone rename. This graph has a different model checksum from the original
// V50 snapshot despite retaining the same on-disk `isDeleted` column. These
// types must never follow active schema edits. A genuine released-binary
// install-over remains the external compatibility proof.
extension MerianReleasedActiveSchemaV50 {
    @Model
    ...
    final class LocalScanRecord {
        ...
        var capturedMediaEntries: [MerianReleasedActiveSchemaV50.CapturedMediaEntry]? = []
        ...
        var collections: [MerianReleasedActiveSchemaV50.ScanCollection]? = []
        ...
        init(
            ...
            collections: [MerianReleasedActiveSchemaV50.ScanCollection]? = [],
    ...
    final class ScanCollection {
        ...
        var scans: [MerianReleasedActiveSchemaV50.LocalScanRecord]? = []
        ...
        init(
            ...
            scans: [MerianReleasedActiveSchemaV50.LocalScanRecord]? = []
        ) {
            self.id = id
            self.name = name
