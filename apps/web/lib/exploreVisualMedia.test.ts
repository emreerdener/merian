...
test("orders visual media canonically and excludes audio", () => {
...
test("uses the hero only when no public visual media is available", () => {
  assert.deepEqual(buildExploreVisualSlides({
    ...
    referenceImages: [],
  }), [{ kind: "image", url: "https://media.merian.app/hero.webp", source: null }]);
});
