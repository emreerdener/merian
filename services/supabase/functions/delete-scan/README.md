...
- **`db.ts`**: Fetches the scan, normalized `scan_media_assets.thumbnail_url`
  values, and post-owned `explore_post_media.thumbnail_url` values before any
  cascade can erase those references. This keeps deterministic spectrogram
  objects discoverable for R2 cleanup even if only one lifecycle surface was
  updated. It then performs the bounded scan-row delete and bubbles relational
