...
Deno.test("community identification note and reasoning trim empty text to null", () => {
  assertEquals(normalizeCommunityNote("  look at wing venation  "), "look at wing venation");
  assertEquals(normalizeCommunityNote("   "), null);
  assertEquals(normalizeCommunityReasoning("\nnot enough detail\n"), "not enough detail");
...
Deno.test("community identification search query collapses spacing and enforces length", () => {
  assertEquals(normalizeCommunitySearchQuery("  red   tailed hawk "), "red tailed hawk");
...
Deno.test("community identification boolean parser rejects non-booleans", () => {
  assertEquals(normalizeCommunityBoolean(undefined, "is_genus_best_possible"), false);
