...
For video scans with `scans.captured_media`, `source_media_id` is resolved
through the same manifest-aware source list used by `share-scan-to-explore` and
`get-explore-composer-media`, keeping the playback `.mp4` paired with its
poster thumbnail instead of relying on sampled frame indexes.
