...
Idempotently follows or unfollows a visible Explore author profile. This
endpoint backs the `Follow` / `Following` button in `ExploreAuthorProfileSheet`.
...
- `is_following` is required and must be a boolean. `false` is valid and means
  unfollow.
- Authentication is resolved by `withEdgeHandler`; the request body cannot
  choose the follower user ID.
...
The response is the authoritative state the client should apply after any
...
Unfollow deletes the `(follower_user_id, followee_user_id)` row. It does not
require the target profile to still be visible, so a viewer can always remove a
stale follow relationship after a block, shadowban, or profile visibility
change.
...
- Follow notifications are postless (`post_id = NULL`) and never trigger APNs
  push delivery.
