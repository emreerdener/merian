...
Deno.test("visual media prompt adds accepted still-photo focus context", () => {
  ...
  assert(prompt?.includes("tentative attention hint"));
  assert(prompt?.includes("not as proof of the intended subject"));
  assert(prompt?.includes("incidental or background biology"));
  assert(!prompt?.includes("Prioritize that region"));
});
