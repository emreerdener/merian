...
const manualRevocationDeliveryMigrationUrl = new URL(
  "../../migrations/20260807034322_deliver_legacy_apple_revocation_instructions.sql",
  import.meta.url,
);
...
Deno.test("legacy Apple instructions require provider-confirmed delivery before Auth", async () => {
  const sql = normalized(
    await Deno.readTextFile(manualRevocationDeliveryMigrationUrl),
  );

  for (
    const fragment of [
      "DO $manual_revocation_delivery_schema$ BEGIN",
      "LOCK TABLE auth.users IN SHARE ROW EXCLUSIVE MODE",
      "LOCK TABLE internal.account_deletion_jobs IN SHARE ROW EXCLUSIVE MODE",
      "manual_revocation_delivery_status TEXT NOT NULL",
      "manual_revocation_delivery_resolved_at TIMESTAMPTZ",
      "manual_revocation_delivery_provider_id TEXT",
      ...
      "REVOKE ALL ON TABLE internal.apple_manual_revocation_delivery_requirements FROM PUBLIC, anon, authenticated, service_role",
      "REVOKE ALL ON TABLE internal.apple_manual_revocation_delivery_attempts FROM PUBLIC, anon, authenticated, service_role",
      "REVOKE ALL ON TABLE internal.apple_manual_revocation_delivery_events FROM PUBLIC, anon, authenticated, service_role",
      "'pending', 'accepted', 'delivery_delayed', 'retry_required', 'delivered', 'unverifiable', 'not_required'",
      "'prepared', 'accepted', 'delivery_delayed', 'delivered', 'retry_required'",
      ...
      "'email.delivered', 'email.delivery_delayed', 'email.bounced', 'email.failed', 'email.suppressed'",
      "manual_revocation_event_id_conflict",
      "manual_revocation_delivery_id_conflict",
      "DELETE FROM internal.apple_manual_revocation_delivery_requirements AS requirement",
      "account_deletion_manual_delivery_guard_missing",
      ...
      "account_deletion_manual_delivery_required",
      ...
      "manual_revocation_delivery_rpc_acl_invalid",
  ...
  const compatibilityEnd = sql.indexOf(
    "COMMENT ON FUNCTION public.complete_account_deletion_manual_revocation_delivery",
    compatibilityCompletion,
  );
  const compatibilityRoutine = sql.slice(
    compatibilityCompletion,
    compatibilityEnd,
  );
  assertStringIncludes(
  ...
  assert(
    !compatibilityRoutine.includes(
      "DELETE FROM internal.apple_manual_revocation_delivery_requirements",
    ) &&
      !compatibilityRoutine.includes(
        "SET manual_revocation_delivery_status = 'delivered'",
      ),
    "The prior acceptance-only RPC must remain a fail-closed rollout fence.",
  );

  const eventStart = sql.indexOf(
    "CREATE OR REPLACE FUNCTION public.record_account_deletion_manual_revocation_event",
  );
  const deliveredBranch = sql.indexOf(
    "IF p_event_type = 'email.delivered'",
    eventStart,
  );
  const deliveryGuardRemoval = sql.indexOf(
    "DELETE FROM internal.apple_manual_revocation_delivery_requirements AS requirement",
    deliveredBranch,
  );
  const deliveryCommit = sql.indexOf(
    "SET manual_revocation_delivery_status = 'delivered'",
    deliveryGuardRemoval,
  );
  const finishStart = sql.indexOf(
    "CREATE OR REPLACE FUNCTION public.finish_account_deletion_attempt",
  );
  const deliveryEvidence = sql.indexOf(
    "delivery_event.event_type = 'email.delivered'",
    finishStart,
  );
  const finishFence = sql.indexOf(
    "account_deletion_manual_delivery_required",
    finishStart,
  );
  const terminalUpdate = sql.indexOf(
    "SET user_id = NULL",
    finishStart,
  );
  assert(
    eventStart >= 0 &&
      deliveredBranch > eventStart &&
      deliveryGuardRemoval > deliveredBranch &&
      deliveryCommit > deliveryGuardRemoval &&
      deliveryEvidence > finishStart &&
      finishFence > finishStart &&
      terminalUpdate > finishFence,
    "Only a matching journaled delivery event may remove the Auth fence, and finalization must revalidate that evidence.",
  );

  const acceptanceStart = sql.indexOf(
    "CREATE OR REPLACE FUNCTION public.record_account_deletion_manual_revocation_acceptance",
  );
  const acceptanceEnd = sql.indexOf(
    "COMMENT ON FUNCTION public.record_account_deletion_manual_revocation_acceptance",
    acceptanceStart,
  );
  const acceptance = sql.slice(acceptanceStart, acceptanceEnd);
  assert(
    acceptance.indexOf("event.event_type = 'email.delivered'") <
        acceptance.indexOf(
          "DELETE FROM internal.apple_manual_revocation_delivery_requirements",
        ) &&
      acceptance.lastIndexOf("RETURN 'delivery_pending'") >
        acceptance.indexOf(
          "SET manual_revocation_delivery_status = 'accepted'",
        ),
    "Send acceptance must wait unless an out-of-order matching delivery event is already durable.",
  );
