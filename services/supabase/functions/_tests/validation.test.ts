...
Deno.test("UUID validation — crypto.randomUUID() output always passes", () => {
    for (let i = 0; i < 10; i++) {
        assert(isValidUuid(crypto.randomUUID()), "crypto.randomUUID() must pass validation");
    }
});
...
Deno.test("exportScope — 'global' is accepted", () => {
    assert(isValidExportScope("global"));
});
...
function handleInsertError(errorCode: string | undefined): "queued" | "already_pending" | "error" {
