...
Authenticated comment emoji reactions. `index.ts` validates a bounded request
and uses the viewer from the shared Edge handler; `db.ts` delegates to the
shared reaction data adapter. Database RPCs enforce authorization and target
visibility.
