...
Deno.test("TestFlight scan recovery documentation preserves retry and legacy-share boundaries", async () => {
  ...
  for (
    const fragment of [
      ...
      "The current test-only follow-up advances those fixtures to",
...
Deno.test("joined scan reliability documentation preserves critical contracts", async () => {
  ...
  for (
    const fragment of [
      ...
      "The current test-only follow-up advances both queue/job mirrors to the scheduled wake",
      "normalizes repository-source whitespace before pinning the guarded token sequence and occurrence count",
      "Strict no-cache lint is clean for both modified test files",
      "Release archive independently passed at 239,079,424 bytes",
      "`da104ff95f8ea3796d95718a8dd80756ecda5bd8d78cf1993b285907f1d9abb1`",
      "contained no Debug-only UI seed markers",
      "A new hosted committed descendant must execute the corrected complete unit target",
      ...
      "the queued-scan UI smoke did not produce acceptance evidence",
      "A new hosted committed descendant containing that correction must pass",
