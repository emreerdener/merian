...
Deno.test("joined scan reliability documentation preserves critical contracts", async () => {
  ...
  for (
    const fragment of [
      ...
      "The exact-SHA follow-up completed 23 of 24 catalog files and passed all 30 inline/video assertions",
      "fixture-only `42702` ambiguity at `ingestion-intent setup`",
      "Pending one further exact-SHA replay after the `fixture_scan_id` rename; all 24 files must pass",
