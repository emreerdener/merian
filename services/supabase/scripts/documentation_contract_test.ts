...
Deno.test("credential documentation preserves the format-aware header contract", async () => {
  ...
  for (
    const fragment of [
      ...
      "After an inbound request exactly matches a valid configured key, downstream work uses that matching configured copy rather than an unrelated preferred source.",
...
Deno.test("Edge route availability docs preserve the gateway-handler boundary", async () => {
  ...
  assertStringIncludes(
    runbook,
    "separately probes sixteen customer-critical routes",
  );
  assertStringIncludes(
    backend,
    "all sixteen customer-critical scan, signing, share-state, Explore, Field Chat, Community, identity-handoff, and deletion routes",
  );
  assertStringIncludes(
    incident,
  ...
  assert(
    ...
    "The Supabase operations README must use the current sixteen-route authorization inventory.",
  );
  assertStringIncludes(
    backend,
