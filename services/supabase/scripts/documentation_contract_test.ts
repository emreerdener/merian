...
Deno.test("joined scan reliability documentation preserves critical contracts", async () => {
  ...
  for (
    const fragment of [
      ...
      "The complete-manifest path also requires its retry-state reset to save",
      ...
      "Final Deno discovery run: 1,357 passed, 0 failed",
      "optional PostgreSQL integration bodies self-skipped because this sandbox denied localhost TCP",
      "172 assertions passed across 26 migration contract files",
      "the revised fixture now has 21 assertions, adding locked-geoprivacy defaulting",
      "full 89-function fleet from 144 changed files",
      "including all nine required scan functions",
      "shuffled-plan and fail-stop fixtures passed",
      "The exact-SHA follow-up completed 23 of the then-current 24 catalog files and passed all 30 inline/video assertions",
      "fixture-only `42702` ambiguity at `ingestion-intent setup`",
      "all 25 current files must pass",
      "Workflow run 1551 for commit",
      "Workflow run 1552 for commit",
      "22 of 24 catalog files",
      "Runs 1549–1552 and the exact-SHA follow-up stopped before production mutation",
