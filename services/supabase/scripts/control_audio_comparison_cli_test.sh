...
assert os.environ.get('IDENTIFICATION_AUDIO_COMPARISON_V1') is None
...
export GITHUB_WORKFLOW_REF=emreerdener/merian/.github/workflows/identification-audio-comparison.yml@refs/heads/main
...
plan=re.search(r'AUDIO_COMPARISON_PLAN_SHA256\s*=\s*"([a-f0-9]{64})"', (base/'comparison/plan.ts').read_text()).group(1)
bundle=re.search(r'IDENTIFICATION_BUNDLE_SHA256\s*=\s*"([a-f0-9]{64})"', (base/'deploymentIdentity.ts').read_text()).group(1)
print(json.dumps(dict(version=1,ownerId='11111111-1111-4111-8111-111111111111',planSha256=plan,backendBundleSha256=bundle,startsAt='2026-09-23T22:00:00.000Z',expiresAt='2026-09-24T00:00:00.000Z'),indent=2))
PY
)"
