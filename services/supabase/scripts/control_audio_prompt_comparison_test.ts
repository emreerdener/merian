...
import {
  controlAudioPromptComparison,
  PROMPT_COMPARISON_PROJECT,
  PROMPT_COMPARISON_SECRET,
  PromptComparisonCliError,
  promptComparisonCliFailure,
  promptComparisonConfiguration,
  PromptComparisonControlError,
  type PromptComparisonControlRuntime,
  requirePromptComparisonWorkflow,
} from "./control_audio_prompt_comparison.ts";
...
} from "../functions/identify-multimodal/comparison/promptAssignment.ts";
...
Deno.test("CLI diagnostics retain only allowlisted categories, never upstream data", () => {
  ...
  for (const [code, expected] of cases) {
    assertEquals(
      promptComparisonCliFailure(encode({
        _tag: "Error",
  ...
  ) assertEquals(promptComparisonCliFailure(encode(value)), "nonzero_exit");
  assertEquals(
    promptComparisonCliFailure(new TextEncoder().encode(config().slice(0, -1))),
    "nonzero_exit",
  );
  assertEquals(
    promptComparisonCliFailure(new Uint8Array(1_048_577)),
    "output_limit",
  );
});
...
function fake() {
  ...
  const runtime: PromptComparisonControlRuntime = {
    now: () => state.now,
    list: () => {
      state.lists++;
      return Promise.resolve(
        state.digest === null ? [] : [{
          name: PROMPT_COMPARISON_SECRET,
          value: state.digest,
        }],
      );
    },
    set: async (input) => {
      ...
      assertEquals(input, JSON.stringify(promptComparisonConfiguration(input)));
...
Deno.test("comparison control leaves absent configuration disabled without private input", async () => {
  for (const operation of ["inspect", "deactivate"] as const) {
    ...
    const evidence = await controlAudioPromptComparison(
      request(operation, ""),
      runtime,
    );
...
Deno.test("comparison activation is bounded, redacted and idempotent; deactivation verifies absence", async () => {
  const { state, runtime } = fake();
  const active = await controlAudioPromptComparison(
    request("activate"),
    runtime,
  );
  assertEquals(active.status, "active");
  assertEquals(active.target, PROMPT_COMPARISON_PROJECT);
  assertEquals(active.block, 1);
  ...
  const again = await controlAudioPromptComparison(
    request("activate"),
    runtime,
  );
  ...
  assertEquals(state.sets.length, 1);
  const disabled = await controlAudioPromptComparison(
    request("deactivate"),
    runtime,
  );
...
Deno.test("an expired stored configuration is inert and remains removable after plan or bundle changes", async () => {
  const { state, runtime } = fake();
  await controlAudioPromptComparison(request("activate"), runtime);
  state.now = Date.parse("2026-09-24T20:00:00.000Z");
  const expired = await controlAudioPromptComparison(
    request("inspect"),
    runtime,
  );
  ...
  await assertRejects(() =>
    controlAudioPromptComparison(request("activate"), runtime)
  );
  assertEquals(state.sets.length, 1);
  const old = config({
  ...
  state.digest = await sha256Hex(
    JSON.stringify(promptComparisonConfiguration(old)),
  );
  const result = await controlAudioPromptComparison({
...
Deno.test("comparison activation rejects invalid identity, window and deployment inputs before mutation", async () => {
  const invalid = [
    ...
    config({ version: 2 }),
    ...[0, 4, 1.5, true, "1", [1], null].map((block) => config({ block })),
    ...
    config({ startsAt: "2026-09-24T18:00:01.000Z" }),
    config({ expiresAt: "2026-09-24T18:00:00.000Z" }),
    config({ expiresAt: "2026-09-24T20:00:00.001Z" }),
    config({ startsAt: "2026-09-24T18:00:00Z" }),
    config({ startsAt: "2026-09-32T22:00:00.000Z" }),
  ];
  for (const raw of invalid) {
    const { state, runtime } = fake();
    await assertRejects(() =>
      controlAudioPromptComparison(request("activate", raw), runtime)
    );
    assertEquals(state.sets.length + state.unsets, 0);
  }
  const { state, runtime } = fake();
  ...
  state.now = Date.parse("2026-10-24T00:00:00.000Z");
  await assertRejects(() =>
    controlAudioPromptComparison(
      request(
        "activate",
        config({
          ...
          expiresAt: "2026-10-24T01:00:00.000Z",
        }),
      ),
      runtime,
    )
...
Deno.test("comparison control preserves an unrelated replacement for every operation", async () => {
  for (const operation of ["inspect", "activate", "deactivate"] as const) {
    ...
    const error = await assertRejects(
      () => controlAudioPromptComparison(request(operation), runtime),
      PromptComparisonControlError,
    );
...
Deno.test("activation diagnostics distinguish a child failure from a failed post-set verification", async () => {
  for (
    ...
    const { runtime } = fake();
    runtime.set = () => Promise.reject(new PromptComparisonCliError(kind));
    const error = await assertRejects(
      () => controlAudioPromptComparison(request("activate"), runtime),
      PromptComparisonControlError,
    );
...
Deno.test("late activation and failed deletion cannot report verified success", async () => {
  ...
  state.digest = await sha256Hex(
    JSON.stringify(promptComparisonConfiguration(config())),
  );
  runtime.unset = () => Promise.resolve();
  const failed = await assertRejects(
    () => controlAudioPromptComparison(request("deactivate"), runtime),
    PromptComparisonControlError,
  );
...
Deno.test("malformed remote digest inventory fails closed", async () => {
  for (
    const rows of [
      null,
      {},
      [{ name: PROMPT_COMPARISON_SECRET, value: "bad" }],
      [
        { name: PROMPT_COMPARISON_SECRET, value: "a".repeat(64) },
        { name: PROMPT_COMPARISON_SECRET, value: "a".repeat(64) },
      ],
    ]
  ) {
...
Deno.test("control output remains compatible with runtime owner and expiry enforcement", async () => {
  ...
  const scanId = await audioPromptComparisonScanId(1);
  const body = {
    user_id: OWNER,
...
Deno.test("each selected block admits exactly its twelve slots and cannot replace the prior block", async () => {
  const { state, runtime } = fake();
  for (const block of [1, 2, 3]) {
    const raw = config({ block });
    if (block > 1) {
      const beforeSets = state.sets.length;
      await assertRejects(() =>
        controlAudioPromptComparison(request("activate", raw), runtime)
      );
      assertEquals(state.sets.length, beforeSets);
      const removed = await controlAudioPromptComparison(
        request("deactivate", config({ block: block - 1 })),
        runtime,
      );
      assertEquals(removed.cleanup, "verified_absent");
    }
    const active = await controlAudioPromptComparison(
      request("activate", raw),
      runtime,
    );
    assertEquals(active.block, block);
    const configuration = state.sets.at(-1)!;
    for (let slot = 1; slot <= 36; slot++) {
      ...
      if (Math.ceil(slot / 12) === block) assert((await resolve()) !== null);
      else await assertRejects(resolve);
    }
  }
  assertEquals(state.sets.length, 3);
  assertEquals(
    (await controlAudioPromptComparison(
      request("deactivate", config({ block: 3 })),
      runtime,
    )).cleanup,
    "verified_absent",
  );
...
Deno.test("comparison control CLI rejects local, fork, branch and other workflow contexts", () => {
  ...
  for (const key of Object.keys(env)) {
    assertThrows(() =>
      requirePromptComparisonWorkflow({ ...env, [key]: "invalid" })
    );
  }
});
...
Deno.test("comparison workflow keeps private input and recovery behind the reviewed Production control", async () => {
  const text = await Deno.readTextFile(
    ".github/workflows/identification-audio-prompt-comparison.yml",
  );
  ...
  assert(text.includes("comparison/promptPlan.ts"));
  ...
  assert(
    text.includes(
      "IDENTIFICATION_AUDIO_PROMPT_COMPARISON_V1: ${{ secrets.IDENTIFICATION_AUDIO_PROMPT_COMPARISON_V1 }}",
    ),
  );
