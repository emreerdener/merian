-- Explore hashtags are stored as normalized post metadata so campaigns,
-- events, and BioBlitzes can match a tag without parsing public notes.
CREATE TABLE IF NOT EXISTS public.explore_post_hashtags (
    post_id UUID NOT NULL REFERENCES public.explore_posts(id) ON DELETE CASCADE,
    tag TEXT NOT NULL,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    PRIMARY KEY (post_id, tag),
    CONSTRAINT explore_post_hashtags_tag_format
        CHECK (tag ~ '^[a-z0-9][a-z0-9_]{1,39}$')
);

CREATE INDEX IF NOT EXISTS idx_explore_post_hashtags_tag_post_id
    ON public.explore_post_hashtags(tag, post_id);

ALTER TABLE public.explore_post_hashtags ENABLE ROW LEVEL SECURITY;

COMMENT ON TABLE public.explore_post_hashtags IS
    'Normalized public hashtag edges for Explore posts. Tags omit the leading #.';
