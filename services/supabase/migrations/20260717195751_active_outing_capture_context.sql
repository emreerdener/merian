-- A deliberately small, private read model for the live capture surface.
-- The Edge Function supplies the verified account id and returns only the
-- current unfinished checklist prompts. It never exposes scan evidence.

CREATE INDEX IF NOT EXISTS idx_user_field_trips_capture_context
    ON public.user_field_trips(user_id, started_at DESC)
    WHERE completed_at IS NULL AND hidden_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_field_trip_challenge_participants_outing
    ON public.field_trip_challenge_participants(user_field_trip_id);
...
AS $$
    ...
    accessible_outings AS (
        ...
        WHERE uft.user_id = self_id
          ...
          AND NOT EXISTS (
              SELECT 1
              FROM public.field_trip_challenge_participants participant
              WHERE participant.user_field_trip_id = uft.id
          )
...
COMMENT ON FUNCTION public.get_field_trip_capture_context(UUID) IS
    'Private service-role capture context. Returns only accessible active standard outing prompts and aggregate progress; never scan ids, media, location, notes, or evidence.';
