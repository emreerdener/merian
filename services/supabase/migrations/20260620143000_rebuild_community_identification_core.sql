...
-- Recreate the public Explore read RPCs from a shared projection instead of
-- text-patching PG_GET_FUNCTIONDEF output. The previous dynamic rewrite was
-- too sensitive to production function formatting and geoprivacy-era variants.
           ...
           OR (
               ...
               AND (
                   (
                       ...
                       AND cards.public_longitude BETWEEN viewer_longitude - search_window.longitude_delta
                           AND viewer_longitude + search_window.longitude_delta
                   )
                   OR (
                       viewer_longitude - search_window.longitude_delta < -180
                       AND (
                           cards.public_longitude >= viewer_longitude - search_window.longitude_delta + 360
                           OR cards.public_longitude <= viewer_longitude + search_window.longitude_delta
                       )
                   )
                   OR (
                       viewer_longitude + search_window.longitude_delta > 180
                       AND (
                           cards.public_longitude >= viewer_longitude - search_window.longitude_delta
                           OR cards.public_longitude <= viewer_longitude + search_window.longitude_delta - 360
                       )
                   )
               )
           )
