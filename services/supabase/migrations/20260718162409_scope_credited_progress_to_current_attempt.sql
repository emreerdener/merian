-- Keep credited progress scoped to completion rows inserted by the current
-- application attempt. A re-identified scan may already have credited an item
-- in an earlier level, and those historical rows must not duplicate the
-- response or supply the toast's progress ring.
...
BEGIN
    ...
    DROP TABLE IF EXISTS pg_temp.field_trip_challenge_new_completions;
    ...
    INSERT INTO pg_temp.field_trip_challenge_new_completions(participation_id, item_id)
    SELECT participation_id, item_id
    FROM inserted;
    ...
    WITH touched_participations AS (
        ...
        FROM public.field_trip_challenge_participants p
        JOIN pg_temp.field_trip_challenge_new_completions new_completion
            ON new_completion.participation_id = p.id
        WHERE p.user_id = self_id
    ),
    ...
    credited_progress AS (
        SELECT
            ...
            COUNT(DISTINCT all_completions.item_id)::INTEGER AS completed_count
        FROM public.field_trip_challenge_participants p
        JOIN pg_temp.field_trip_challenge_new_completions credited_completion
    ...
    newly_completed AS (
        ...
        JOIN public.field_trip_challenge_item_completions comp
            ON comp.participation_id = p.id
           AND comp.item_id = new_completion.item_id
        JOIN public.field_trip_checklist_items fci
