-- A report flag is moderation workflow state, not an identification rewrite.
-- Keep public reasoning visible for flagged posts while continuing to suppress
-- reasoning after the user overrides the AI identification.
...
COMMENT ON FUNCTION public.get_explore_post_detail(UUID, UUID) IS
  'Returns privacy-safe public Explore detail. Report flags do not hide AI reasoning; user identification overrides do.';
