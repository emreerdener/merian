...
`apikey` and Bearer. Credential sources are classified independently so a stale
migration slot cannot veto an exact valid key, and production rollout requires
the synchronized Edge secret's stored digest to match the selected key. Exposed
