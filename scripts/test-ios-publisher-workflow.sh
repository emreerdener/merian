...
assert_contains 'ios-build-and-test.yml/runs?head_sha=$GITHUB_SHA&per_page=20' "$workflow"
...
assert_contains "Wait for exact-SHA iOS production readiness" "$workflow"
assert_contains "runs-on: ubuntu-latest" "$workflow"
assert_contains "contents: none" "$workflow"
assert_contains "max_attempts=61" "$workflow"
assert_contains 'sleep "$poll_seconds"' "$workflow"
assert_contains "Timed out after 30 minutes waiting" "$workflow"
assert_contains "completed with conclusion" "$workflow"
assert_contains 'needs.readiness.outputs.green_run_id' "$workflow"
