...
- **Historical Weather Backfilling**: Uses a dedicated `fetchHistoricalContext` executing dynamic temporal range queries purely using `WeatherKit` by querying `.hourly(startDate: date, endDate: date.addingTimeInterval(3600))`. This physically maps backwards in time securely allowing Library uploads to query exact environment conditions years locally without external servers.
- During coordinate fetching loops, `EnvironmentContextManager` actively defers its CPU locks *beneath* structural Optimistic UI executions manually deployed within `CameraViewModel`. Crucially, bounds a strict OS `Task { Task.sleep }` 2.0-second delay natively actively cancelling and resuming the coordinate callback returning `nil` preventing satellite acquisitions in terrible forest domains from triggering infinite hanging "Acquiring coordinates..." UI loops permanently deadlocking the async flow out-of-bounds natively. 
