...
- Maps physical optical sensor boundaries natively through `activeFormat.videoMaxZoomFactor`, actively adjusting dual-wide equivalent focal constraints (`0.5x`, `1x`, `2x`) down to exact Apple hardware parameters natively scaling the `captureDevice.videoZoomFactor`.
