...
- **Vision streaming paragraph** (`VisionStreamingParagraph`): A private `Text`-based view reading `inferenceEngine.visionAnalysisText`. Characters appear one-by-one (22 ms per char) as each Vision pass completes. Uses `.contentTransition(.opacity)` + `.animation(.easeOut(duration: 0.25), value: text)` for a fluid reveal. The paragraph only appears when `visionAnalysisText` is non-empty or `isVisionStreaming` is `true`; it enters via `.opacity.combined(with: .offset(y: 8))`.
