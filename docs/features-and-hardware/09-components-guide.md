...
- **Threshold sourcing**: `CandidatesCard` computes the threshold internally via `MerianConfig.confidenceBands(forInferenceTier: inferenceTier).diagnosticTrigger` (`0.96` Flash / `0.85` Pro) for display copy, matching the server-side strip in `supabase/functions/identify/thresholds.ts` exactly.
