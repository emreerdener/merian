...
- **`SwipeableCandidateCard`**: The core structural view for the individual species cards. Integrates `SimilarSpeciesImageFetcher` to asynchronously load the species' Wikipedia/GBIF visual, masking it under a 3-stop vertical gradient `[.clear, .black.opacity(0.6), .black.opacity(0.85)]` to strictly protect typographic contrast. Uses `.ultraThinMaterial` for the match-percentage confidence pills. Renders an `OriginalCapturePiPView` in the bottom-right corner so users can compare the candidate species against their own capture without leaving the swipe interface.
- **`OriginalCapturePiPView`**: A private 58×76 pt picture-in-picture thumbnail embedded inside `SwipeableCandidateCard`. Resolves the user's original scan image in priority order: (1) `inferenceEngine.activeDisplayDatas.first` decoded off-thread via `Task.detached` + `ImageDownsampler` at 512 px; (2) `inferenceEngine.validHistoricImagePaths.first` via `AsyncLocalImageView` for historical scans; (3) solid black placeholder if neither source is available. Shows a `ProgressView` while the detached decode is in-flight. Styled with a 12 pt rounded corner, white 40% opacity border, and a subtle shadow.
...
## 14. Model Info Section: `ModelInfoSection`
...
The skeleton state rendered inside `InsightSheetView` while `InferenceEngine.isProcessing == true` and `speciesData == nil`. Replaces the former fullscreen `ScanningOverlayView` — the insight sheet now opens immediately when the user submits a scan and shows this view until the AI response arrives, then cross-fades into the real biological or non-biological content via a `.easeInOut(duration: 0.35)` animation keyed on `isProcessing`.

- **Phase text header**: Reads `inferenceEngine.scanningPhaseText` (driven by `InferenceEngine.classifySubjectLocally` / `startPhaseRotation`). Each phrase change is animated with an asymmetric spring crossfade using SwiftUI's `id()` identity trick — the `Text` view re-creates itself on each new phrase, triggering `.asymmetric(insertion: .opacity + offset(y:4), removal: .opacity + offset(y:-4))`. The static subtitle "Just a moment..." remains fixed below.
- **Skeleton cards**: Three `SkeletonCardBlock` views (heights 140 / 220 / 160 pt) using `Color(uiColor: .secondarySystemBackground)` fill and a subtle `0.04` shadow to sit correctly on the insight sheet's `systemBackground` content area. The entire skeleton group pulses between opacity `0.6` and `1.0` via a 1.2-second `easeInOut.repeatForever` loop driven by a single `@State private var pulse: Bool` flag set on `.onAppear`.
- **No internal spacer**: `AnalyzingContentView` intentionally omits a trailing `Spacer` — `InsightContentView.contentCards` provides the universal `Spacer(minLength: 40)` outside the if/else routing block, preventing double-spacing in analyzing mode.
