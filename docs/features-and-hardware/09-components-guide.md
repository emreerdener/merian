...
- **Review state priority**: Before evaluating confidence bands, the badge checks `isFlagged`, `userIdentificationOverride`, and `userConfirmedIdentification` passed from `InsightHeader`. If `isFlagged == true`, the badge rigidly locks to "Under Review" (orange, `flag.fill`). Else, if `userIdentificationOverride != nil`, the badge shows "Your ID" (indigo, `person.fill.checkmark`). If `userConfirmedIdentification == true`, it shows "Confirmed" (green, `checkmark.seal.fill`). All review states render regardless of `confidenceScore` (including zero for historical scans).
