...
- **Tap behavior**: The gallery accepts an optional `onSpeciesSelected` callback. The Insight sheet and Explore post detail page pass this callback to open `SpeciesDictionaryPageView` for the tapped lookalike, preferring `SimilarSpeciesEntry.speciesId` when present and falling back to `scientificName`; each route also carries a zero-PII species dictionary analytics entry point. Species dictionary usages omit the callback so their similar-species section stays read-only in V1.
- **Relation explanation**: When a lookalike relation includes `reason`, the card shows that concise visual explanation below the scientific name. If `reason` is missing but `visualTraits` are present, the card falls back to the first shared traits. Historical cached entries without metadata still render as the original name/image card.
