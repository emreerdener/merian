...
the `fetchAndApplyEnrichment` facade. `InferenceSpeciesEnrichmentCoordinator`
owns independent scope loading, current-presentation application, and bounded
retry; `InferenceSpeciesEnrichmentService` owns wire-to-domain mapping, and the
admitted `InferenceHydrationPersistenceService+Live` write stores it in
...
The domain-neutral pager, page identity value, zoom host, and pagination dots
shared with Field Trips live under `Core/UI/Components/MediaCarousel`. Insight
retains mixed-media assembly and projects its feature-owned page identity into
that Core UI boundary.
