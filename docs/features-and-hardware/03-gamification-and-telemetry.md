...
| `ImageFocusDetectionCompleted`            | `trackImageFocusDetection(durationMilliseconds:outcome:areaBucket:)` | `durationMs`, `outcome`, optional `areaBucket: "small"/"medium"/"large"`                                                         | Capture Shared's bounded Vision focus request completes, times out, fails, or is cancelled        |
...
Focus-detection telemetry is limited to nonnegative duration, a coarse outcome,
and an optional `small`/`medium`/`large` accepted-area bucket. It must not
include image content, normalized region coordinates, filenames, scan IDs,
species values, or account identifiers. The current outcome allowlist is
`accepted`, `no_subject`, `low_confidence`, `invalid_geometry`, `area_rejected`,
`ambiguous`, `error`, `timeout`, and `cancelled`.
