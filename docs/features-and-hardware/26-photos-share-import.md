...
`apps/ios/Merian/Configuration/Info.plist` registers one
...
that access is active, and stops access immediately afterward. It never
persists the source URL, a Photos asset identifier, or a bookmark. The inbox is
excluded from device backups and accepts at most eight pending receipts.
...
and crop lifecycle owns it; cancelling the required crop removes the staged
item and does not restore the inbox receipt.
...
- default single-capture mode auto-submits when "Confirm scan submission" is
  disabled;
- confirmation-enabled mode leaves the cropped photo in the Active Scan
  toolbar until the user taps Identify;
