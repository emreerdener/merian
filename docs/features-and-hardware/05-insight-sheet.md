...
| `BiologicalView` | Full biological result: taxonomy, ecology badges, confidence, Wikipedia, Pro Field chat, habitat/distribution, observation patterns, and lookalike diagnostic. Cards enter with a hardware-gated staggered animation via `CardEntranceModifier`. |
...
| `InsightChatCard` | Pro-only Field chat card inserted immediately after `OverviewCard` for completed biological, non-human insights with a stable scan id. Free users see a locked row that opens the existing paywall. Pro/trial users get three context-aware chips plus a compact text composer and saved message history. `InsightChatViewModel` owns loading, sending, deleting, offline state, rate-limit/error copy, duplicate-send client ids, and chip generation; chat state stays out of `InsightSheetViewModel` except for paywall presentation. Sending is online-only and talks to `/insight-chat`; the card never sends raw image bytes, cloud image URLs, or public Explore data. |
