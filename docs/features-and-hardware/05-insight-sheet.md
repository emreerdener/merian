...
2. **Server-side strip** (`supabase/functions/identify/index.ts`): After Gemini returns, `index.ts` calls `diagnosticTriggerForTier(tier)` from `supabase/functions/identify/thresholds.ts` — the canonical source of truth for all threshold constants. If `confidence_score >= diagnosticTrigger`, the `candidates` array is cleared to `null` before the response is sent to the client and before the `insertScan` DB write. This is the sole gate: Gemini is not asked to self-suppress, the server enforces the rule unconditionally. **Thresholds**: `0.96` Flash (== `FLASH_STRONG`), `0.85` Pro (== `PRO_STRONG`). `MerianConfig.flashConfidence.diagnosticTrigger` and `MerianConfig.proConfidence.diagnosticTrigger` in the iOS client mirror these values and must be kept in sync.
