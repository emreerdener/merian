...
confidence data, ecological context, and media for every scan. It is presented
as a sheet modal from `CaptureWorkspaceView` when
`CaptureWorkspaceViewModel.activeSheet == .insight`.
...
When an offline scan completes while `InsightSheetView` is open, the sheet must
**transition to results without dismissing**. The key mechanism:

`LibraryView` uses `.sheet(isPresented: $isQueuedSheetPresented)` with a
separate `scanToManage: QueuedScanContext?` state. This decouples sheet
presentation from the context — clearing `scanToManage` does not close the
sheet.
...
`LibraryView.onChange(of: queuedScans.map(\.id))` also runs in parallel: when a
scan leaves the queued array, it checks whether a `LocalScanRecord` exists for
that ID. If yes, it sets `scanToManage = nil` (keeping
`isQueuedSheetPresented = true`) so `InsightSheetView` receives `nil` for
`queuedScan` and its internal transition runs. If no record exists (scan was
deleted or failed), it sets `isQueuedSheetPresented = false` to dismiss the
sheet.
...
intact while the sheet transitions from queued/analyzing state to results.
