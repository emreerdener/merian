...
ID exists, its delete button is transparent, disabled, accessibility-hidden, and
has no UI-test identifier. Binding the exact queued row fades that same button
in over 0.2 seconds and exposes `InsightQueuedDeleteButton`; tapping it retains
the existing queued **Cancel upload & delete** confirmation and deletion
semantics. No second toolbar item or sheet relayout is introduced.
