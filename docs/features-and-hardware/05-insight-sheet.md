...
| `SpeciesObservationChartsCard` | Reusable Swift Charts card rendered after Habitat & Distribution for known biological species. Uses `SpeciesObservationStatsViewModel` to combine local on-device `LocalScanRecord` aggregates with cached global public iNaturalist stats from `/species-observation-stats`. Tabs are Seasonality, History, Life Stage, and Sex. Sex is external-only in V1. |
...
- `SpeciesObservationStatsViewModel` aggregates local `LocalScanRecord` rows by
  effective species match using `captureDate ?? timestamp`.
- Local Seasonality, History, and Life Stage are computed on-device. Local Sex
  is intentionally absent in V1.
