...
| `AnalyzingContentView` | Skeleton state shown inside `InsightSheetView` while inference is in-flight. Displays the live `inferenceEngine.scanningPhaseText` headline (animated with an asymmetric spring crossfade as phrases rotate) plus three pulsing skeleton card blocks. Removed after `isProcessing` becomes `false` via an animated `.opacity` transition. |
