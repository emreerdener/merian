...
| File | Role |
|---|---|
| `ViewModels/InsightSheet/InsightSheetViewModel.swift` + extensions | `@Observable @MainActor final class` — the root file owns stored state, init/reset, and `UIState`. Extension files isolate display, lifecycle, record lookup/mutation, field notes, Explore sharing, media export, and name-preference behavior while preserving the scan-ID/snapshot safety boundary. |
...
| `Views/InsightSheet/InsightSheetView+Toolbar.swift` | Keeps toolbar assembly out of the root sheet body. It wires queued-scan trash handling, collection actions, report/reanalyze/review/confirm actions, and bottom-toolbar visibility using view-model capability flags and record snapshots. |
| `Modifiers/EmbeddedInsightNavigationModifiers.swift` | Hosts the feature-local back-swipe, navigation swipe enabler, and species-dictionary destination modifier used by embedded Insight navigation stacks. |
| `Models/InsightPresentation.swift` | Defines `InsightPresentationStyle` and `ScanInsightRoute`, the value-only presentation/navigation surface used across Insight, Explore, and Species Dictionary links. |
| `Models/InsightToolbarRecordSnapshot.swift` | Captures the toolbar's value snapshot of a `LocalScanRecord` before modal/share/delete boundaries so UI work does not retain a deleted SwiftData model. |
...
| `InsightChatSheet` | Bottom-sheet Field chat for completed biological, non-human insights with a stable scan id. The Insight bottom toolbar shows the chat entry for eligible scans; non-Pro users hit the existing paywall, while Pro/trial users open a sheet with the scan's single saved chat, contextual chips, message history, and an anchored composer. `InsightChatViewModel` owns conversation loading, sending, offline read-only state, deterministic unavailable hiding, rate-limit/error copy, durable failed-send retry/edit state, duplicate-send `client_message_id` values, chip generation, answer feedback, and chat-to-notes summary draft loading; chat state stays out of `InsightSheetViewModel` except for presentation/action hooks. Assistant replies show local source chips, safety refusals render as `Safe field guidance`, and reply actions include copy, Ask Community, lookalike comparison, append to field notes, review confidence, feedback, and reviewed append-only chat summaries. Sending is online-only and talks to `/insight-chat`; the sheet never sends raw image bytes, cloud image URLs, exact GPS, or public Explore data. The Edge Function builds its prompt from stored private scan/species text, including review/provenance state, observed traits, ecological annotations, species group tags, and image/capture-quality metadata. |
...
**State model**: `CandidateSwipeSession` in `Models/CandidateReview` owns `originalCandidates`, `remainingCandidates`, `confirmedCandidate`, and `isExhausted`, plus the skip/reject/confirm/restart decisions. `CandidateSwipeModal` keeps animation, drag offsets, dismissal timing, paywall routing, and sheet wiring in SwiftUI.
