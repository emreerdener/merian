...
| `Media/Carousel/`                                                        | `ImagesCarousel` is the stable Insight composition entry. Feature `Models` own selection, focus, image-origin, and availability policy; `Builders` derive ordered pages; `InsightMediaPlaybackDependencies` adds Insight telemetry and feedback naming to the Core playback dependency value; `Playback` owns only the inline Insight video and pause coordinator; and `Pages`, `Components`, and `Animation` retain feature-specific rendering, gestures, and analysis motion. The fullscreen gallery, audio page, generic video chrome, pager, and gallery values live in Core UI. Carousel views perform no networking or direct live-service lookup, and every production Carousel Swift file stays below the 600-line review guard.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `BiologicalView`                                                         | Full biological result: taxonomy, ecology badges, confidence, Wikipedia, habitat/distribution, observation patterns, and lookalike diagnostic. Cards enter with a hardware-gated staggered animation via the feature-owned `InsightCardEntranceModifier`; the hardware eligibility value arrives through `InsightContentDependencies`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `InsightHeader`                                                          | Scrollable header with species name, description, `ConfidenceBadge`, and the Core render-only `ModelTierBadge`. `BiologicalView` prepares its complimentary/upgrade presentation from environment-provided entitlement owners and Insight Shell owns the paywall. For dog/cat scans with a displayable `SpeciesData.petIdentification`, the large headline uses the pet label while the subtitle remains the scientific name only; otherwise the header uses normal common-name/scientific-name behavior. It deduplicates a subtitle that exactly matches the title and routes alternative-name taps to the shared `NamePickerSheet`. Non-biological results display `Non-biological`. Optional Apple Vision analysis text appears first, then cross-fades to Gemini reasoning while feature-injected feedback preserves the existing entrance behavior.                                                                                                                                                                                                                                                                                                                                                                                                       |
...
Each appearance begins one explicit Insight presentation session. Close,
programmatic dismissal, refinement dismissal, a binding transition to false, and
a completed embedded-navigation back swipe all end that session before
navigation continues. Ending is idempotent and invalidates scan-bound
generations, delayed onboarding, sharing operations, Field-trip contribution
loads, and media-export task ownership. An async dependency that ignores
cancellation therefore cannot publish feedback or another UIKit presentation
over a later Insight.
...
- `SpeciesObservationStatsViewModel` invokes initializer-injected
  `SpeciesObservationStatsDependencies`. The live Services adapter creates
  `SpeciesObservationStatsDatabaseActor` for filtered SwiftData projection
  fetches and delegates local normalization/bucketing to
...
- A per-load generation token prevents late local/public responses from
  overwriting a newer species request. Local and public failures are handled
  independently.
...
`.task` asks the generation-fenced `SimilarSpeciesImageFetcher` to load. Its
injected `SimilarSpeciesImageService` resolves the Wikipedia/GBIF waterfall and
delegates permitted bitmap downloads to `LocalImageLoader`. In all failure cases
the card stays in the gallery — it is never removed. The only exclusion rule is
a blank `scientificName` (truly invalid server data), filtered in
`validEntries`.
...
and typed dismissal-request wiring in SwiftUI. `CandidateReviewViewModel` owns
the scan-and-generation fence and delegates review mutations to
`CandidateReviewDependencies`; the modal performs no direct engine mutation or
live-service lookup. A staged request is unavailable until the modal has
actually dismissed, is consumed only once, and is invalidated if the current
scan disappears or no longer matches its captured presentation generation.
...
Candidate live thumbnails downsample off-main and check cancellation before
publishing. The original-capture picture-in-picture decode is additionally keyed
to and revalidates the engine presentation generation, so a late image cannot
appear on a replacement scan.
...
`BiologicalView` derives a `ModelTierBadgePresentation` for the Results
countdown; it is not limited to the Possible-match upgrade case. For a verified
unpaid complimentary account, it shows the server-reported scans remaining and
routes the Core render-only badge action to Insight Shell's `PaywallView`. On
the third usable Pro result it immediately changes to the exhausted upgrade
action, while the current and historical Pro result content remains fully
available.
...
The feature observes environment-provided RevenueCat and versioned
`EntitlementManager` state and never decrements a local credit counter. The Core
badge resolves neither singleton and owns no sheet. See
...
After deletion is accepted, the view invokes the same session-aware dismissal
path used by close, back, and queued deletion. The view model owns no SwiftUI
...
`InsightSheetViewModel+MediaExport` maps the current immutable scan/media
snapshot into requests for the Core `MediaExportService`. Save and share
preparation each have a retained task plus operation UUID and must still own the
scan ID, sheet generation, and presentation session before committing UI state.
The service handles two export paths:

- **Save to Photos**: `MediaSaveRequest` carries live capture data, valid
  historic local or approved cloud image/video sources, and the existing
  approved reference-photo URL. The private service actor saves images through
  `PhotoLibraryManager.saveImageManual`, while retained playback clips use the
  file-backed `.video` resource path. Cloud URLs require HTTPS and the exact
  `media.merian.app` host. The ephemeral, cookie-free export session refuses a
  redirect away from that host before following it and revalidates the final
  response URL. Completion returns separate photo and video counts so alerts and
