...
| `GBIFHeatmapMapView` | SwiftUI `View` that composites two images to render a full-world GBIF occurrence heatmap. (1) An off-screen `MKMapSnapshotter` renders the Apple Maps base layer at full world extent — bypassing `MKMapView`'s interactive zoom-out cap that prevented showing the whole world in a small card. (2) The GBIF density zoom-0 tile (`/0/0/0@2x.png`) — a single 512 px PNG covering the entire world in Web Mercator — is fetched and drawn on top. Both images share identical Web Mercator world extent so they composite in alignment. Snapshot and tile are fetched concurrently via `async let`. Rendered inline in `HabitatAndDistributionCard` for **all users** when `speciesData.gbifTaxonKey` is non-nil. Non-interactive. Always displays full-world distribution. |
