...
| `InsightChatSheet` | Bottom-sheet Field chat for completed biological, non-human insights with a stable scan id. The Insight bottom toolbar shows the chat entry for eligible scans; non-Pro users hit the existing paywall, while Pro/trial users open a sheet with the scan's single saved chat, contextual chips, message history, and an anchored composer. `InsightChatViewModel` owns conversation loading, sending, offline state, deterministic unavailable hiding, rate-limit/error copy, duplicate-send client ids, and chip generation; chat state stays out of `InsightSheetViewModel` except for presentation hooks. Sending is online-only and talks to `/insight-chat`; the sheet never sends raw image bytes, cloud image URLs, or public Explore data. The Edge Function builds its prompt from stored private scan/species text, including review/provenance state, observed traits, ecological annotations, species group tags, and image/capture-quality metadata. |
