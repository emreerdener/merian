...
When the AI's confidence falls below the tier-specific `diagnosticTrigger` threshold (`0.88` for Free/Flash, `0.80` for Pro), the `candidates` array is forwarded to the client and displayed as a `CandidatesCard` in `BiologicalView`.
...
2. **Server-side strip** (`supabase/functions/identify/index.ts`): After Gemini returns, `index.ts` computes `diagnosticTrigger = userTier === "pro" ? 0.80 : 0.88`. If `confidence_score >= diagnosticTrigger`, the `candidates` array is cleared to `null` before the response is sent to the client and before the `insertScan` DB write. This is the sole gate: Gemini is not asked to self-suppress, the server enforces the rule unconditionally.
