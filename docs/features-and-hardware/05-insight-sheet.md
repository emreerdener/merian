...
**Exhausted state** (`exhaustedStateContent`): Shown when all candidates have been swiped away. Displays the original scan thumbnail and two action controls stacked vertically:
1. `SlideToConfirm(label: "Confirm species", color: .green)` — calls `onConfirmOriginal()` and dismisses
