...
| `InsightChatSheet` | Shared bottom-sheet Field chat. For completed biological, non-human Insights, the toolbar preflights ownership through `/check-scan-status` and uses `/insight-chat`. Other users' Explore post details reuse the same floating `FieldChatToolbarButton`, sheet, and view model with the `/explore-post-chat` source. Every Explore thread belongs only to the requesting viewer; its empty state says `This Field chat is private and visible only to you.` and omits additional technical disclaimers. Non-Pro users open the existing paywall. The sheet renders saved messages, AI-generated quick prompt chips with local fallback, offline read-only state, durable failed-send retry/edit, safety refusal styling, answer feedback, and an anchored composer. Owner-only Insight actions such as append-to-notes, feature feedback, identification review, and reanalysis are disabled for Explore threads. The backend retains the full context boundary: Insight uses private owned scan text, while Explore uses only privacy-filtered public post/detail and Species Dictionary text. Neither path sends raw image bytes or cloud image URLs to Field chat. |
