...
- Natively exposes an `isGuestUser` property (mapped to `currentUser?.isAnonymous`) allowing features like the `ProfileTabView` to selectively render Native Apple Authentication loops instead of prematurely forcing "Sign Out" buttons on ghost boundaries.
...
- Evaluates `isProActive` booleans via `.customerInfo()` natively checking for active entitlements across `pro` and `7_day_pass` identifiers seamlessly updating SwiftUI. The `PlanCard` dynamically observes this property natively within the Profile header, instantly redrawing the subscription tier card to definitively reflect the current state (e.g., Naturalist UNLIMITED SCANS vs Explorer 2 SCANS DAILY) and surfacing the `PaywallView` sheet fluidly.
