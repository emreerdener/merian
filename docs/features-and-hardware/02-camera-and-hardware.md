...
- **Visibility toggle**: Gated by `camera.isZoomSupported && zoomSliderVisible`. The `zoomSliderVisible` flag (`@AppStorage(UserDefaultsKeys.zoomSliderVisible)`, default `true`) lets users hide the overlay entirely via Camera Settings without affecting the underlying zoom functionality.
...
- Rendered by `MainOverlayView` only when `activeScanImages.count < 2` (hidden during multi-image staging). Hidden entirely when `activeScanImages.count >= 2` via `MainOverlayView`'s conditional — the same guard that hides the `ZoomSliderView`.
...
- Hidden (via `CameraRootView`) when `activeScanImages.count >= 2` (during multi-image staging).
...
- **Top** — `MediaModeToggle` (hidden during multi-image staging).
- **Bottom** — `PhotoLibraryButton` · `CaptureButton` · `FlashButton` row (hidden during multi-image staging), `MainTabBar` or `ActiveScanToolbar` depending on whether images are staged.
...
**Session lifecycle**: `onChange(of: captureMode)` stops the camera session when switching to `.audio` and restarts it on return to `.visual` (unless `isAnalyzingFullscreen` is active). This prevents the `AVCaptureSession` from running unnecessarily while the audio page is visible.
