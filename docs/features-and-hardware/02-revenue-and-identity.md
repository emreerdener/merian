...
- **Premium Insights Hook**: `PremiumInsightsCard` is embedded in `InsightSheetView` via `BiologicalView`. For Pro users, enrichment loads automatically 2–3 seconds after each scan via `InferenceEngine.fetchAndApplyEnrichment` — no user action required. A shimmer loading skeleton is displayed while `isPremiumLoading` is `true`. If a Pro user has no enrichment data (e.g. the network call failed), they see a "Retry" button. For free users, the card renders as a glassmorphism paywall offering a frictionless $2.99 7-Day Pass via the RevenueCat `weekly` package. On purchase, `triggerEnrichment()` calls `InferenceEngine.fetchAndApplyEnrichment`, which hits the `/enrich-scan` Edge Function — the server enforces the Pro tier gate, so enrichment is only returned once the entitlement is active.
