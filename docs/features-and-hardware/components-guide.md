...
- Delegates user profile interactions cleanly out to `UserProfileView.swift` (and subsequently `SettingsView.swift`) preventing monolithic Main Actor View bloat by isolating `StatCardView` modules securely into their own feature domains.
...
- Key logical container boundaries:
  ...
  - `InsightTaxonomyHeader` & `InsightTaxonomyTree`: Repositions analytical boundaries cleanly. The Tree drops legacy scrolling components directly rendering a sleek vertical key-value dictionary natively matching `.glassCard` styling parameters explicitly parsing terminal SPECIES vectors structurally.
  - `InsightEnvironmentCard` & `InsightConservationCard`: Newly forged modular `.ultraThinMaterial` capsules safely projecting strict IUCN status states and environmental variables natively onto localized blocks explicitly mapping `iucnRedListStatus` and `ecologyType` classifications into structural UI components completely bypassing traditional massive text payloads.
