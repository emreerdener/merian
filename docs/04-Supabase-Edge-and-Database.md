...
2. **R2 Integration**: The Deno edge node extracts the `image_storage_urls` array natively from Supabase securely via `.select()`. It processes the string keys natively instantiating `AwsClient` (via `aws4fetch`) executing a physical `DELETE` payload natively erasing bytes off Cloudflare R2 permanently. Note: The pipeline directly uses the explicitly stored full API URLs (e.g. `https://<account>.r2.cloudflarestorage.com/<bucket>/...`) natively without manually rebuilding or concatenating the endpoint string, mathematically preventing malformed double-namespace URI paths (e.g. `.../merian/merian/...`) that would otherwise cause the payloads to be stranded in the bucket.
