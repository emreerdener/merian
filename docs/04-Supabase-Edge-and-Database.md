...
1. Receives a structured payload containing the `r2ObjectKey` and environmental constraints (like GPS Coordinates, GPS Elevation, Semantic Location, Device Locale, Current Month, Time of Day, relative humidity, UV index, Pitch, Heading, Flash Fired state, and Weather conditions) from the iOS native client. Concurrently, it extracts the `user.id` cryptographically from the `Authorization` header JWT via a completely local `jose.jwtVerify()` signature check against the `SUPABASE_JWT_SECRET`. **Note:** Because Merian utilizes modern Asymmetric ES256 Ghost sessions (`signInAnonymously`), the default Supabase `verify_jwt` Edge Middleware is explicitly disabled inside `supabase/config.toml` (`verify_jwt = false`). The Deno handlers now explicitly run manual standard SSR Auth extraction logic natively bypassing RTT (Round Trip Time) polling delays to GoTrue entirely, completely stopping 401 Rejections from Edge Gateway boundaries and entirely eradicating IDOR vulnerabilities.
