...
10. **Warm-Start Latency Optimization**: Across all Edge Functions (`identify`, `generate-upload-urls`, `safe-delete`, `block-user`), the `supabaseAdmin = createClient(...)` instantiation is explicitly hoisted dynamically out of the `serve(...)` request handler loop into the global script scope. Multi-file AWS requests executing array mappings (`delete-scan`, `generate-upload-urls`) resolve structurally via `Promise.allSettled()` and `Promise.all()` to flatten latency entirely across concurrent HTTP boundaries natively.

## The Webhook Node (`revenuecat-webhook`)
Merian natively ties into RevenueCat's Webhook architecture to drive async tier boundaries (`pro` or `free` migrations). Because Deno Enforces strict 10-second processing limits, the webhook node explicitly delegates heavy S3 operations (like bulk moving payloads from `/free/` into `/pro/` buckets) off-the-edge by natively deferring them via `EdgeRuntime.waitUntil(promise)`. This guarantees an instant `HTTP 200` back to RevenueCat, bypassing critical 504 Timeout Retries completely while keeping Cloudflare Edge logs clean and free of verbose spam.
