...
2. **Zero-OOM Edge Strategy (Ephemeral Networking)**
   ...
   - The Deno Edge node fetches the image securely from R2 as an ArrayBuffer and uploads it directly to the Gemini `generativelanguage.googleapis.com/upload/v1beta/files` File API endpoint. This completely bypasses Base64 stringification, thereby permanently preventing V8 Out-Of-Memory (OOM) crashes across large payload constraints.
