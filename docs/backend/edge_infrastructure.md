...
Merian relies firmly on strict physical IDFV identity string hashing natively mapped across devices.

- Physical `scans` table entries map natively to the iOS Keychain `user_id` mapped string.
- **Open Access Layer**: All scans designated with the `geoprivacy = 'open'` enum and `is_live_capture = true` correctly appear naturally via Global Discovery feeds.
- **Restricted**: Any `private` scans natively reject `.select()` operations exclusively unless the requested IDFV token physically corresponds to the original scanner natively mapping the row.

## The Cloudflare R2 "Quarantine" Cycle

1. Images natively land in a Cloudflare bucket inside a `staging/` prefix automatically via frontend pre-signed URLs.
2. The `identify` Edge securely extracts this binary byte stream and pipes it to Gemini LLMOps thresholds natively.
