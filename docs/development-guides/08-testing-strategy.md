...
- **`SpeciesDictionaryTests.swift`**: Validates the public dictionary response decoder, legacy no-`schema_version` compatibility, species-ID-preferred request payloads, view-model success/not-found/error states, and the `MerianNetworkClient` 10-minute in-memory memoization path for recently opened species pages. Test setup resets the singleton cache whenever a mocked `URLSession` is injected so serialized network tests do not share stale dictionary entries.
