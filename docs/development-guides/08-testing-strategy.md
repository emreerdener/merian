...
- **`CameraManagerTests.swift`, `CaptureWorkspaceViewModelTests.swift`**: Validates UI state routing logic. `CaptureWorkspaceViewModelTests` sends `AppEventPublisher.shared.send(.appDidEnterInactivePhase)` (not `NotificationCenter`) to trigger the modal-reset path — `CaptureWorkspaceViewModel` subscribes to `AppEventPublisher`, not legacy `NSNotification`. Asserts that the "Legacy Viewfinder" toggle (`isLiveInferencePaused`) disables background thread evaluation when set.
