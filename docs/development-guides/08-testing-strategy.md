...
- **`MigrationPlanTests.swift`**: Two-tier structural guard for the SwiftData migration plan.
  ...
  - `migrationFromV30ToV32DoesNotCrash`: creates a real V30 disk store then reopens with the full migration plan targeting V32. On iOS 26+, even a lightweight migration triggers validation of ALL custom stages in `MerianMigrationPlan.stages` via `NSCustomMigrationStage.init(migratingFrom:to:)`. Two bugs cause equal-model crashes: (1) extension-declared `@Model` classes whose metadata resolves to the global type; (2) `ScanCollection` typealias carrying a relationship to a prior schema's `LocalScanRecord` by Swift type identity. The disk-based test covers this execution path that the in-memory test misses. The test validates the V31 (`isFlagged`) and V32 (`isUploaded`) lightweight additions in one pass. Update the version pair to the current pair when a new schema is added. Run both tests on an iOS 26 simulator on every schema bump.
...
- **`OfflineQueueManagerTests.swift`**: Mocks queue payload insertions.
  ...
  - **`replayInferenceForUploadedScans` pickup (`testReplayInferencePicksUpUploadedScans`)**: Inserts an `isUploaded=true, isDeleted=false` scan, calls `replayInferenceForUploadedScans()` with `isOnline=true`, and asserts the scan ID is present in `activeScanUploadIds`. Validates the happy-path connectivity-restore replay.
  - **`replayInferenceForUploadedScans` deduplication (`testReplayInferenceSkipsActiveScanIds`)**: Pre-seeds the scan ID into `activeScanUploadIds` before calling `replayInferenceForUploadedScans()`, then asserts the set count does not grow. Guards against duplicate inference tasks for a scan whose replay is already in-flight.
...
- **`CompositeLibraryTests.swift`** (`merianTests/Features/Scans/`): Validates the bounding behaviors of the composite `ScansGrid` that renders both `OfflineQueuedScan` and `LocalScanRecord` items in the same `LazyVGrid`.
  ...
  - **`isUploaded` Default (`testIsUploadedDefaultsFalse`)**: Asserts a freshly constructed `OfflineQueuedScan` has `isUploaded == false`, so existing pre-V32 records are never treated as already staged in R2 after the lightweight migration.
