...
- Do not use `UIImage(contentsOfFile:)` or `UIImage(data:)` on user-selected originals, avatar picks, scan media, or share/export flows. Route previews through `ImageDownsampler.downsampledUIImage(...)` or `downsampledSendableImage(...)`, and route payloads through bounded `CGImage` encoding so the full raster never enters RAM.
