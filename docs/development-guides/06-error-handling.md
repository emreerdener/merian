...
| A Field Chat send is canceled after its pending bubble enters `sending`                                                               | If the exact subject and request generation are still current, clear `sending` and mark that same bubble failed and retryable under its original UUID. If the subject was replaced or cleared, stale cleanup mutates nothing.                                                                                                                                                                                                              |
| Field Chat readiness or prompt work is replaced, cleared, or goes offline before scheduled work starts                                | Obsolete readiness returns false without surfacing an error into the replacement subject. The newest prompt trigger owns the result, loading always clears for its generation, and an unavailable refresh preserves existing or deterministic fallback prompts.                                                                                                                                                                            |
...
| SwiftData store corruption at startup                                                                                                 | Verified-corrupt store artifacts and the atomic support manifest are archived as one rollback-protected operation. Only then is the store-aware persistent open retried once and "Library Repaired" shown; an archive or retry failure enters safe mode                                                                                                                                                                                    |
| SwiftData schema migration failure at startup                                                                                         | Eligible legacy store artifacts and the atomic support manifest are archived under `store-rescue/` as one rollback-protected operation. Only then does a fresh persistent current-schema store open and show "Library Rebuilt" with `legacy_store_rescued` telemetry; an archive or fresh-open failure enters safe mode                                                                                                                    |
| Non-corruption `ModelContainer` startup failure                                                                                       | Local store files are not moved; app boots a plan-free in-memory `CurrentSchema` safe mode with a startup notice                                                                                                                                                                                                                                                                                                                           |
...
- `ModelContainer` bootstrap failures now follow a recovery ladder: store-aware
  ...
  → legacy migration rescue → plan-free in-memory `CurrentSchema` safe mode with
  startup notice. The full historical migration plan is validated independently
  rather than reused by that final fallback.
