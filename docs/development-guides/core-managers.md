...
- **Free User Queue Cap & Quota Enforcement**: `enqueueCapture` enforces a hard queue depth cap of `UsageManager.shared.maxFreeScansPerDay` (2) items for free users — if the cap is reached, new items are rejected and any files already written to disk are cleaned up atomically. `syncPendingScans` additionally checks `canPerformScan` before starting any batch and calls `consumeScan()` once per queued item submitted, ensuring the daily quota is respected even when scans complete via the background URLSession path. Pro users have no queue depth cap and bypass the quota check entirely.
