...
- **Dual-path indexing**: Full rebuilds extract `RawScanSnapshot` values from the already-resident `allScans` array on `@MainActor`, then build `SearchableScan` payloads in a detached utility task. Incremental inserts stay on `SearchDatabaseActor`, but fetch their delta through one batch `FetchDescriptor` (`WHERE id IN (...)`) rather than faulting records one-by-one.
