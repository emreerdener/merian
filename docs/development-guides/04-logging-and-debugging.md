...
| DwC-A failure code `source_snapshot_changed` | At least one immutable source member no longer satisfies its creation-time privacy eligibility, or its durable invalidation fence changed. The worker makes the job terminal and deletes any uploaded/staged archive. If Resend accepted an in-flight request immediately before completion lost the fence, the email may exist but its archive is revoked and deleted. | Treat this as expected privacy enforcement, not a retryable provider outage. Verify the job is failed, its source DTOs and private staged URL were purged, and the attempt-fenced R2 object is absent. The user must create a new coherent snapshot; never clear invalidation metadata or republish the old URL. |
...
| `{"event":"dwca_export_queue_health",...}` or a failed `DwC-A Export Queue Health Monitor` run | The oldest claimable job reached the five-minute warning/15-minute critical age, backlog reached 25/100 jobs, a claim expired, or the monitor could not read aggregate health. `queue_drained` means no work is currently claimable and can coexist with leased or delayed backlog. | Inspect `runtime_deadline_reached`, `step_limit_reached`, backlog/due counts, claim counts, and oldest-due age. Repair database, R2, or Resend availability and let oldest-due claim-fenced draining recover. Do not clear claims, rewrite cursors, or raise the step ceiling to silence the alert. |
