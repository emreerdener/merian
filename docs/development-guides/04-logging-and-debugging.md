...
| `account_deletion_attempt_deferred` with `manual_delivery` stage                                                                                               | The legacy Apple instruction could not be submitted to Resend or its claim-fenced acceptance record could not be persisted. Supabase Auth remains protected by the delivery requirement before and after send acceptance; logs contain only job/stage/bounded code, never the recipient or provider body.                                                                                                                       | Repair `RESEND_API_KEY`, the verified sender/domain, provider reachability, or database persistence, then let the scheduled worker retry. Do not treat acceptance as delivery, delete Auth, edit delivery state/provider email ID, remove the private Auth fence, or copy the recipient into diagnostics.                                                                                                                                                                              |
| Legacy Apple webhook health reports delayed, retry-required, unverifiable, invalid-signature, stale-attempt, or persistence failure                                                                                          | No signed `email.delivered` event has authoritatively released the current attempt. Delayed delivery continues waiting; bounced, failed, and suppressed attempts require a new attempt. Auth remains fenced. Duplicate/out-of-order events are normal inputs; signature, correlation, and durable reduction determine authority.                                                                                          | Verify `RESEND_WEBHOOK_SIGNING_SECRET`, endpoint URL/event subscriptions, sender/private-relay configuration, and bounded reducer health. Let claim-fenced retry or a new durable attempt proceed. Never replay an old event onto a new attempt, log the recipient/raw payload, forge delivery, or remove the Auth fence manually.                                                                                                                                                   |
