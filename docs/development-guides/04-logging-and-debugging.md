...
| Opening Scan Library repeats signing, successful R2 PUTs, `failed_retryable / background_ingestion_failed` status, and one-second wakes without any Identify request | Build 1.0.2 (235)'s upload/status deadlock erased retry authority on upload success, then treated the same failed generation as permanently server-owned. Repeated refresh logs are a symptom, not retry ownership. | Require the durable `server_retryable_failure` latch/count to survive re-stage, then verify one delayed preflight sends Identify. Release the corrected iOS state machine; do not clear queue rows, rotate UUIDs, or make the presentation refresh loop dispatch work. |
...
| A new scan shares successfully but an existing missing row reports `media_reconciliation_abandoned`, then restore signing returns generic 503 | The primary Explore route is healthy; the legacy record needs guarded owner/media repair. The terminal reason is eligible only with its matching service-written post-result dead letter. | Under restricted access, prove the exact owner/scan job and `failed_scan_ingestions` provenance, then verify migration `20260729173000_recover_media_abandoned_owned_scans.sql` and signer/status/share versions. Unproven abandonment and policy terminal state must remain closed. |
