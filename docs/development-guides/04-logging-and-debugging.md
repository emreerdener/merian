...
| Local auth log mentions Apple credential state after a revocation notification                                                                                       | iOS received Apple's notification and queried the active provider-specific subject. `.authorized` preserves the session; a non-authorized result or lookup failure clears only that still-matching local Apple session. The log omits the provider subject and is not a server revocation receipt.                                                                                                                                                | For a lookup failure, check device connectivity and Apple service health, then require normal Apple reauthentication. Do not mark the account-deletion provider stage complete, infer that `/auth/revoke` succeeded, or copy Apple identity values into diagnostics. An authorized-state debug message needs no remediation.                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
