...
| `Merian_AccountDeletionRecoveryCapability_v1`            | `AccountDeletionRecoveryCapabilityStore` via `KeychainManager` (`WhenUnlockedThisDeviceOnly`)       | Protocol-v2 JSON envelope with distinct random 256-bit recovery/acknowledgement authorities; installed proofless `intake_pending` recovery alone may create the legacy raw 256-bit format. Edge stores only domain-separated hashes; iOS verifies creation and removal around its identity-free durable phase marker |
...
`Core/Security/KeychainKeys.swift` is the single registry for every exact
app-owned Keychain key string in this table. The registry is naming authority,
not a codec or lifecycle owner. `Core/Preferences/UserDefaultsKeys.swift`
similarly owns exact defaults strings, including the identity-free deletion
phase and manual Apple-revocation notice keys.
...
is the sole envelope codec and verified secure-storage owner for the deletion
capability. The package's Models and Stores separately own installed recovery
phase values, the read-back-verified defaults marker, and the manual provider
notice. Endpoint payloads and workflow sequencing remain in Core Network.
...
verified removal. Auth/server/provider phase order and deletion routing remain
in Core Network Auth coordinators. `PurchaseIdentityHandoffCoordinator` owns the
completion task keyed by destination, Auth generation, and transition owner and
the proof-removal boundary; `SupabaseManager` supplies its live Auth, provider,
entitlement, journal, endpoint, and diagnostics effects.
`PurchaseIdentitySessionCoordinator` and `PurchaseIdentityReadinessCoordinator`
share one injected handoff boundary for readiness admission. Each successful
durable-journal read republishes its derived pending value to RevenueCat's
synchronous mutation fence. If either journal is unreadable, both coordinators
publish pending state before returning, so missing in-memory state cannot reopen
paid mutation while durable authority is indeterminate.
...
`GhostProfileMergeCoordinator` keys completion by the exact target UUID and
optional Auth-transition owner, so an older cancelled task cannot clear the
handle or proof for a newer session. A preparation capability returned by the
server is persisted and suppression is closed before caller cancellation may end
subsequent Auth work. `GhostProfileMergeWorkflow` checks cancellation before the
first server effect and between server completion, purchase synchronization,
local-evidence synchronization, and final proof removal.
...
device response boundary. Cancellation after compatibility preparation's final
SDK-session read cannot report success, and the already-persisted proof remains
recoverable. `PurchaseIdentitySourceHandoffCoordinator` also revalidates the
exact Auth transition or its unowned account-work lease after the initial
suspended SDK read and before remote cancellation in either lane. Stable
cancellation repeats that fence after the final SDK read and before durable
proof removal. A failed pre-dispatch fence sends no cancellation; a failed final
fence retains the journal and republishes the pending mutation fence. A
different permanent session, an old anonymous session, an expired claim, a
malformed journal, or a temporarily unreadable Keychain keeps purchase, restore,
redeem, provider linking, and ordinary principal resolution closed. A readable
pre-protocol-3 client-only value at the same key can be retired only by its
exact restored source and can never authorize a destination.
...
conservative local erasure. For v2, `not_committed` or an unknown proof admitted
outside the installed mixed-domain compatibility state below authorizes
proof-only retirement because destructive commit cannot run without the server
preparation. If another device committed first, the database converts every
preparation into a receipt and recovery returns pending/completed instead. The
device acknowledges with the distinct second proof before retiring the envelope;
expiry blocks inspection, not post-cleanup acknowledgement. When authenticated
intake is definitively rejected with `409 purchase_continuity_pending`, iOS
first persists `capability_rejection_retirement_pending`, then read-after-delete
verifies the unused proof is gone, and removes the marker last. Recovery from
that phase is non-destructive: it cannot sign out or purge local data. This
capability cannot select, restore, or authenticate an account and is never
reused for sign-in or purchase identity.

Installed pre-capability `intake_pending` recovery preserves the legacy hash
domain end to end. If its proof is absent while the exact cached source session
still exists, the recovery coordinator creates and read-verifies one raw
protocol-v1 proof, reuses it across every retry, and calls only the legacy
intake/recovery endpoint. It never serializes a protocol-v2 envelope for a v1
request. Conversely, a proofless `capability_prepared_pending` marker is safe to
cancel by restoring only the exact cached source session: server preparation was
non-destructive and the durable intake marker proves commit had not begun. An
absent or mismatched source session keeps either path blocked.

An earlier client can already have paired a v2 envelope with v1 intake. For an
installed intake/cleanup marker, v2 unknown recovery is therefore inconclusive:
iOS checks v1 recovery with the envelope's recovery value. A positive legacy
match continues cleanup and v1 acknowledgement; another unknown result keeps the
envelope and barrier rather than restoring the account.
