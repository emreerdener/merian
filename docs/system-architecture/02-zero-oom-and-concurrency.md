...
### ImageIO & CoreVideo Autoreleasepool Memory Leaks
...
This identical RAM ceiling violation exists during Apple Vision AI inferences. Executing `VNImageRequestHandler` over CoreVideo pixel buffers inside detached closures leaves enormous megabyte allocations cached until the CPU rotates out the `Task.detached` context. Merian enforces `autoreleasepool { ... }` wrappers across all internal CV layer manipulations, dropping transient spikes completely.

Furthermore, `CIContext(options: [.cacheIntermediates: false])` dynamically instantiates the hardware GPU interface, imposing severe 50-150ms structural stalls. Rather than mapping this onto every CoreVideo evaluation pass (e.g. evaluating post-processing frames alongside the camera viewfinder), execution relies on a globally static reference mounted to the purely detached `InferenceProcessingActor`. Native singleton binding prevents overlapping hardware requests and ensures 120Hz view refresh parity.
