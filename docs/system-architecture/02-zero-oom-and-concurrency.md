...
Search no longer linearly scans every `SearchableScan` on each keystroke. `ScansManager` now maintains a detached-built `SearchIndexSnapshot` containing exact-term, bigram, and trigram posting lists plus precomputed category buckets. Queries intersect those posting lists first, then verify the narrowed candidates against `searchString.contains(...)`. This preserves the previous substring semantics while moving the expensive work from "full library per keypress" to "bounded candidate verification per keypress".
