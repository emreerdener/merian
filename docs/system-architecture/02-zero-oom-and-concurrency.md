...
Gallery, refinement, and avatar preview paths all use bounded ImageIO thumbnailing before constructing a `UIImage`. `StagedImage.displayData` is always a 2048 px re-encoded display payload, never the original mapped file bytes. The profile avatar crop preview uses `ImageDownsampler.downsampledSendableImage(url:maxSize:)` inside detached work and constructs `UIImage(cgImage:)` back on `@MainActor`; direct `UIImage(contentsOfFile:)` reads are not permitted for user-selected originals.
