...
Deferring heavy external SDK boot sequences (like PostHog and Crashlytics) via `DispatchQueue.main.asyncAfter` caused a UI hitch milliseconds after `CaptureWorkspaceView` finished rendering. Merian forces these SDK initializations off the iOS Main Thread by routing them through `DetachedWork.fireAndForget(priority: .background, category: .thirdPartyBootstrap)` combined with `try? await Task.sleep(nanoseconds: 500_000_000)`, guaranteeing zero stutter during app launch while keeping the executor escape explicit.
