...
- **Multi-Image Context Fusion**: The `identify` Edge function accepts an array of `r2ObjectKeys: string[]`. Deno fetches R2 presigned URLs and encodes them to base64 in a serial `for…of` loop — each `arrayBuffer()` is awaited individually before the next begins. Concurrent `.arrayBuffer()` calls (via `Promise.all`) caused back-pressure across V8's I/O loop when both responses resolved simultaneously, producing transient heap spikes inside Deno's 256 MB ceiling. Serial processing keeps memory flat at the cost of a single image-fetch RTT (~10–30 ms). The resulting base64 strings are injected as distinct `inlineData` MIME parts into the Gemini prompt, supporting macro shots alongside wider environmental images.
