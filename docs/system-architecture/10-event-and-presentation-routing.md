...
| `foregroundBiologicalScanCompleted(scanId:)`                 | Invalidation/handoff            | `InferenceLivePresentationCoordinator` commits, then calls `InferenceLiveCompletionCoordinator` → `CaptureWorkspaceView`                                                                | Normalized scan ID; duplicates only re-evaluate prompt eligibility               | Persisted scan/history; a miss is recovered from the next history projection                                                       | Current account scan             |
...
| `publicAuthorIdentityChanged(previousUserId:currentUserId:)` | Invalidation                    | `SupabaseManager`/`ProfileViewModel` → Explore root/detail/author profile and profile scan preview                                                                                      | Current user ID; later identity is authoritative                                 | Auth session and public-profile service; manager emission follows successful target-account/task-ID and exact-session revalidation | Explicit account boundary        |
...
| `speciesDictionary(speciesId:)`                                   | URL/share link                           | Explore Identify/Species and dictionary detail           | Normalized species ID                                                | Public route; canonical UUID validation                      |
| `communityIdentification(requestId:)`                             | push/internal action                     | Explore Identify/Community detail                        | Normalized request ID                                                | Account-sensitive                                            |
...
Applying a refinement route starts a new Capture refinement session. Before
loading the new target's original evidence, `startRefinementScan(with:)` cancels
prior refinement staging, discards the previous staged media and supplementary
description association, and clears pending photo selection and prefetch work.
The route's initial description belongs to that new session. This session reset
is separate from pending-route coalescing: the latest description still wins
when equivalent requests have not yet been applied. See
[Describe mode](../features-and-hardware/11-describe-and-voice-dictation.md) for
staging and submission behavior.
