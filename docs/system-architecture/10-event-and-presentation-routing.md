...
| `exploreShareStateChanged(scanId:postId:)`                   | Invalidation                    | Insight/Explore/Scans share-state mutations → Capture cache writer, Explore/profile previews, Collections, `ScansManager` → Library search coordinator | Normalized scan ID; latest durable share state wins                | Local/cloud publication state; consumers patch or refetch by ID                                                                  | Current account mutation         |
...
| `scanSearchIndexInvalidated(scanId:)`                        | Invalidation                    | tag mutation → `ScansManager` → Library search coordinator                                                                                             | Normalized scan ID                                                 | SwiftData scan; coordinator re-extracts one bounded document                                                                     | Current account scan             |
