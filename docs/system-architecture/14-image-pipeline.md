...
**Cancel handler**: `ActiveScanToolbar`'s cancel action clears all four staging buffers: `activeScanImages`, `activeScannedDatas`, `activeOriginals`, and `activeDisplayDatas`. All four must be cleared together — leaving `activeDisplayDatas` populated causes array-length mismatches on the next capture, where `displayDatas` forwarded to the inference engine would contain stale payloads from the previous aborted session.
