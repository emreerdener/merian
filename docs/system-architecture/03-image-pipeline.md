...
The refinement path now shares that same prepared-image staging helper instead of issuing an eager `Data(contentsOf:)` copy of the stored scan image. `startRefinementScan(from:)` builds a `PreparedStagedImageRequest` with `.memoryMapOriginalFile`, sends it through the injected `PreparedStagedImageLoader`, and then commits the result on the main actor without `requiresCrop`. In production the loader uses `ImageDownsampler` plus `.mappedIfSafe`; in tests the same seam is stubbed to verify request routing and staging state transitions without UI automation. This keeps refinement staging byte-bounded, testable, free from a second full-file copy before the user even captures the supplementary image, and isolated from the user-facing gallery crop gate.
