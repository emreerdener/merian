...
The same file-backed rule now applies to explore-media restore. `MerianNetworkClient.restoreExploreMediaObjectKeys` validates each local image path, derives MIME type from the file extension or file header, and uploads via `upload(for:fromFile:)` with bounded concurrency. The restore path must never materialize every image into `Data` just to populate `httpBody`.
