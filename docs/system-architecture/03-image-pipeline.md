...
The refinement path now shares that same prepared-image staging helper instead of issuing an eager `Data(contentsOf:)` copy of the stored scan image. For refinement, the display payload is preserved as a memory-mapped view of the existing on-disk file (`.mappedIfSafe`) while a fresh tier-correct inference payload is generated from `ImageDownsampler`. That keeps refinement staging byte-bounded and avoids a second full-file copy before the user even captures the supplementary image.
...
**Cancel handler**: `ActiveScanToolbar`'s cancel action clears all staging buffers in `CaptureWorkspaceViewModel` (`StagedCapture.clearAll()`) and resets `InferenceEngine` via `cancelActiveRequest()`, which nils `activeImageData` and clears `validHistoricImagePaths`. All buffers must be cleared together to prevent stale payload mismatches on the next aborted session.
