...
- **Two-Call Identification Architecture**: Every scan follows a two-stage pipeline:
  1. **Vision call** (`identify`): Routes to `gemini-2.5-pro` (Pro) or `gemini-2.5-flash` (free). Returns identification fields only — `scientific_name`, `common_name`, `confidence_score`, `blur_score`, `is_biological_subject`, `is_live_capture`, `is_invasive`, `ecology_type`, `colors`, `insight_data` (containing `ai_reasoning`, `hazard_type`, `regional_status_rationale`), `wikipedia_url`, `wikipedia_extract`, `reference_image_url`. `taxonomy` and `iucn_red_list_status` are only present on Cache Hit (read from `species_dictionary`, not generated by the model). `premium_insights` is only present on Cache Hit for Pro users when habitat or distribution data is already stored. `insight_data.ai_reasoning` is always present for biological subjects — it is the vision model's per-scan reasoning about the specific photo, unique to each submission. `hazard_type` is authoritative from `species_dictionary` on Cache Hit; sourced from the vision model on Cache Miss and then cached. The `hazard_type` column exists only on `species_dictionary`, not on `scans`.
  2. **Enrichment call** (`enrich-scan`): Triggered automatically by the iOS client (`InferenceEngine.fetchAndApplyEnrichment`) for Pro users after the vision call completes. Returns `habitat_description`, `global_distribution_regions`, and `diagnostic_comparison` (if confidence < 0.85). Always uses `gemini-2.5-flash` — never the Pro model. Results are stored at the species level in `species_dictionary`. To avoid a duplicate Flash spend when `identify`'s background task and this call race on a Cache Miss, `enrich-scan` polls `species_dictionary` up to 3 times (2-second intervals) before falling back to a Flash generation call. All enrichment data (habitat, distribution, taxonomy, IUCN) is also collected for free users by the `identify` background task — the `enrich-scan` Pro gate is a retrieval gate, not a collection gate. Free users' scans fully populate `species_dictionary` and benefit Pro cache hits on subsequent scans of the same species.
