...
- **Dynamic Diagnostic Thresholds**: The dynamic presentation of diagnostic data (e.g., lookalikes and confidence hooks) is strictly controlled by the client. The backend Edge Functions (`identify` and `enrich-scan`) blindly index and fetch `similar_species` if it is not already cached in the database. When the Swift client receives this data, `BiologicalView` and `SimilarSpeciesGallery` evaluate the scan's confidence against the user's tier-specific diagnostic threshold (`0.88` for Free/Flash, `0.80` for Pro) using `MerianConfig.confidenceBands(forInferenceTier: inferenceTier).diagnosticTrigger`. This logic visually classifies the matched species data either as an actionable "POTENTIAL LOOKALIKES" interface, or purely informational "SIMILAR SPECIES".
