...
- **Token Throttling (`maxOutputTokens: 1000`)**: All production Edge bindings enforce `maxOutputTokens: 1000` for the vision call. This keeps the model focused on populating the JSON output rather than generating internal reasoning chains that can silently exhaust Edge execution time limits. The budget was raised from 800 to 1000 to accommodate the `ai_reasoning` field, which requires 2–4 sentences of visual justification per scan.
...
- **System instruction trimmed and clarified (~120 tokens)**: The `group_tags` example list was removed (already captured in the schema's `description` field) and verbose phrasing was condensed. The liveness instruction was expanded to explicitly include fossils and preserved specimens as biological subjects with `is_live_capture = false`, preventing Flash from returning generic names for paleontological subjects.
