...
4. If `confidenceScore < 0.85` and the response contains `diagnostic_comparison`, patches `speciesData.diagnosticComparison` in-place. (Note: UI display is further gated by a dynamic tier-based threshold).
...
- **Diagnostic Comparison Threshold (`DIAGNOSTIC_THRESHOLD = 0.85`)**: Diagnostic comparison data (primary rationale, confusing lookalike, key differentiators) is only generated when `confidence_score < 0.85`. This constant is defined in both `identify/index.ts` and `enrich-scan/index.ts`. On the Swift client, `InferenceEngine.fetchAndApplyEnrichment` applies the same 0.85 threshold to write `speciesData.diagnosticComparison`, but the UI in `BiologicalView` applies a stricter, model-locked threshold (`MerianConfig.confidenceBands(forInferenceTier: inferenceTier).diagnosticTrigger`), ensuring Free scans evaluate against Flash thresholds while Pro scans evaluate against Pro thresholds. Diagnostic data is cached at the species level in `species_dictionary`.
