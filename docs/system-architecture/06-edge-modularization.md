...
- **`_shared/mediaBudgets.ts`**: Shared media budget and safety checks. Edge functions must use this module for endpoint JSON body ceilings, audio clip count, inline base64 length, raw byte limits, staged R2 ownership/path traversal, allowed staging content types, and `Content-Length` prechecks before parsing media bodies or allocating ArrayBuffers. Do not reintroduce local `MAX_AUDIO_*` or `MAX_IMAGE_*` constants in individual `index.ts` files unless the function has a truly different domain contract and documents that exception.
