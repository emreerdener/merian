...
- **`_shared/scanMediaAssets.ts`**: Shared scan-media projection helpers. Edge write paths make best-effort `refreshScanMediaAssets(...)` calls after scan inserts or video repair updates, while composer/status readers prefer normalized `scan_media_assets` rows before falling back to `captured_media` and legacy media arrays.
