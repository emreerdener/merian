...
- **`_shared/scanMediaAssets.ts`**: Shared scan-media lifecycle helpers. `/generate-upload-urls` creates staged upload-session rows for scan media, `identify-multimodal` marks those rows promoted/deleted/failed during finalization, and write paths make best-effort `refreshScanMediaAssets(...)` calls after scan inserts or video repair updates. Composer/status readers prefer ready display/playback `scan_media_assets` rows before falling back to `captured_media` and legacy media arrays.
