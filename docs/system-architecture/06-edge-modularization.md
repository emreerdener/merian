...
- **`_shared/scanMediaAssets.ts`**: Shared scan-media lifecycle helpers. Edge write paths make best-effort `refreshScanMediaAssets(...)` calls after scan inserts or video repair updates, while composer/status readers prefer ready display/playback `scan_media_assets` rows before falling back to `captured_media` and legacy media arrays.
