...
The first-scan coordinator preserves complete unowned evidence across creation
and adoption of the anonymous session, binds it only under the same current
account-work lease, and rechecks the lease and synchronization generation after
suspension. Persisted evidence owned by any other account remains ineligible.
...
| `CONSENT-012` | First and later inference now require a freshly fetched adult/Terms/current granted Gemini-head proof for the active account. Complete unowned first-scan evidence remains bindable across anonymous-session adoption only under the same current account-work lease; another account's persisted evidence and stale lease/generation completions remain fenced. Exact handler-owned `403 ai_consent_required` closes an in-memory and durable per-account fence, preserves the queued observation/media in needs-attention, stops automatic inference retry, survives relaunch, and routes a completed user through authoritative restoration to Ready. Reapproval writes fresh evidence whose Gemini grant extends the post-rejection provider head; legacy-ledger decode and account switching remain isolated.                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Exact-SHA clean-install first scan, forced missing-row rejection with zero provider/entitlement consumption, relaunch/media retention, fresh reapproval, same-scan retry success, and distinct `402`/`429` UI execution.                                                                                                   |

For `CONSENT-012`, a canceled preflight remains cancellation and a lost
account-work lease or synchronization generation remains an active-account
change. Those stale-context exits must not persist or publish reapproval. Only a
still-current account whose authoritative synchronization completes without the
required proof, or an exact handler-owned `403 ai_consent_required`, may enter
the durable reapproval path. The focused iOS regression exercises the real
pending-push/read-back/fetch/merge projection and separately invalidates the
generation to verify that distinction; it does not replace the exact-SHA hosted
release evidence listed in the table.
...
remains the observable compatibility facade and combines the restoration and
synchronization cancellation snapshots. `ConsentManagerRuntime` wires the
restoration callbacks, `ConsentCloudSessionCoordinator` owns session/lease
adoption, Ghost rebinding, and inference admission, and
...
| **Supabase Candidate Validation** | Complete main-push validation (optional PR scope) and stable Candidate readiness check, clean-SHA check, pinned tools, formatting/lint, migration replay, every discovered pgTAP catalog, complete Edge/database-concurrency suite, database lint, and advisors all green.                 | Prior main run passed (above); final beta-policy SHA must revalidate.                            |
