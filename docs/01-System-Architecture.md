...
- **Deferred Context Fetch (`EnvironmentContextManager`):** Efficiently manages `CoreLocation` and `WeatherKit` by fetching exact coordinates, altitude (`gpsElevation`), and live condition/temperature (`weatherTemperatureF`) exclusively locked at the exact millisecond of the shutter press, preventing continuous battery drain while providing rich pipeline context. It executes a concurrent `CLGeocoder().reverseGeocodeLocation` to natively map semantic location names (e.g. `San Francisco, CA`) directly into the local SwiftData `MerianSchemaV5` database for UI display. It implements a native 2.0-second background timeout race during `requestLocation()` to completely prevent the "Acquiring coordinates..." UI from locking infinitely on dead hardware satellites deep in the forest. Additionally, it seamlessly backfills historical edge metadata natively (GPS and past WeatherKit conditions) structurally mapping `PHAsset` EXIF data for library uploads prior to execution.
