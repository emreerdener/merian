...
3. **Background Processing & Payload Compression (`URLSessionConfiguration.background`)**
   The manager checks that the queue isn't already syncing and then bounds an explicitly constrained `FetchDescriptor` query with `descriptor.fetchLimit = 5` to chunk processing limits safely preventing edge timeout limits. It generates raw Presigned URLs from `/generate-upload-urls`. Before enqueuing the upload natively, the local 12MP file from `URL.documentsDirectory` is loaded, downsampled using `UIGraphicsImageRenderer` to a maximum dimension of `1024px` with `0.7` JPEG compression limit, and safely cached into a temporary `URL.cachesDirectory` file. This highly compressed payload is cleanly handed directly into a strictly isolated native iOS `URLSession` Background Queue natively. The Operating System assumes full responsibility for byte transmission, massively accelerating upload times over spotty 3G cellular boundaries.
