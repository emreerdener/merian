...
| Insights           | `apps/ios/Merian/Features/Insights/`                                                                                                   | Product-area-first insight result feature. `Shell/` owns the root presentation, completed-scan routes, embedded navigation, root view model, idempotent persisted-completion precedence over stale same-ID queued routes, and generation-keyed result action/Field Notes activation; `Content/` owns biological/non-biological results, queued lifecycle UI, foreground analysis, direct-child-before-parent completion notification, and the shared `ScanningExperienceView` with intrinsic accessibility size around its animated status badge; `IdentificationReview/` owns candidates, confidence UX, bounded Canvas glare drawing, opacity-only label transitions, and single-element accessibility semantics; `Media/` owns carousel/gallery/export work plus per-scan audio boost preferences and playback switching; `FieldNotes/`, `Chat/`, `Sharing/`, `Reporting/`, `Toolbars/`, and `SpeciesReference/` own their named insight subareas; `Toolbars/TopToolbar/` owns collection and audio-listening overflow actions; `Shared/` holds reusable insight chrome.                                                                                                                                                                                                                                                                                                                                                                                          |
