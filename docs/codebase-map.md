...
- `MerianTests/Support/SharedProcessStateTestTrait.swift` provides
  ...
  interceptor overrides and offline-queue fixtures. `SharedProcessStateGate`
  atomically leases all requested resources, cancels queued waiters, and checks
  release tokens. Capture's `OfflineQueueTestCase` uses the same gate across
  XCTest setup/teardown. `OfflineQueueTestState` restores the prior model
  context; individual cases still own async completion and all other state
  restoration. Generic Insight contexts do not configure the shared queue, and
  lifecycle admission tests inject inert maintenance actions. Route and paywall
  tests inject private request sinks instead of owning app-host presentation
