...
unsigned current-SHA Release archive. Release preparation and distribution
provenance are enforced by `scripts/prepare-ios-release.sh`,
`scripts/check-ios-release-prep.sh`,
`scripts/ios-release-source-fingerprint.sh`, and
`scripts/embed-ios-build-provenance.sh`. A prep marker binds the release
version/build to the complete nonignored source snapshot. Its typed local schema
requires the final release commit to descend from `prepared_from_sha`; the typed
CI schema requires exact `source_sha` equality. A clean archive embeds its Git
revision, source fingerprint, and source state. CI verifies those values against
`GITHUB_SHA`. `scripts/export-ios-release.sh` refuses an archive that does not
match the clean checked-out release source, disables Xcode's default automatic
build-number management, and then calls
`scripts/validate-ios-exported-ipa.sh`. That validator requires one
non-symlinked IPA and one unambiguous root app, rechecks its bundle,
version/build, revision, fingerprint, and clean state after signing, and
requires the widget, Messages, and watch app to retain the same version/build.
It hashes the file before and after inspection, rejects an IPA changed during
validation, and reports the stable SHA-256 on a dedicated machine-readable
`ipa_sha256=<64 lowercase hex>` line that must identify the uploaded artifact.
...
| Insights           | `apps/ios/Merian/Features/Insights/`                                                                                                   | Product-area-first insight result feature. `Shell/` owns the root presentation, completed-scan routes, embedded navigation, root view model, idempotent persisted-completion precedence over stale same-ID queued routes, and generation-keyed result action/Field Notes activation; `Content/` owns biological/non-biological results, queued lifecycle UI, foreground analysis, direct-child-before-parent completion notification, and the shared `ScanningExperienceView` with intrinsic accessibility size around its animated status badge; `IdentificationReview/` owns candidates, confidence UX, bounded Canvas glare drawing, opacity-only label transitions, and explicitly labeled native Button semantics; `Media/` owns carousel/gallery/export work plus per-scan audio boost preferences and playback switching; `FieldNotes/`, `Chat/`, `Sharing/`, `Reporting/`, `Toolbars/`, and `SpeciesReference/` own their named insight subareas; `Toolbars/TopToolbar/` owns collection and audio-listening overflow actions; `Shared/` holds reusable insight chrome.                                                                                                                                                                                                                                                                                                                                                                                      |
| Scans              | `apps/ios/Merian/Features/Scans/`                                                                                                      | Product-area-first private scan library. `Shell/` owns the root sheet, pager, toolbar, search chrome, and completed/queued pushed Insight destinations; its queued route retains `QueuedScanContext` while hashing by ID, but the destination treats a persisted same-ID completion as authoritative on every bind. `Library/` owns individual scans, `ScansManager`, the search index, queued-scan snapshots, completion-race lookup, and fresh context hydration; `Collections/` owns collection grids, detail routes, smart collections, and scan selection flows; `NonBiological/` owns the non-biological isolation surface; `Shared/` holds scan grid, thumbnail, empty-state, and deletion UI reused across Scans product areas.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
