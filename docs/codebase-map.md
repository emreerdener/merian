...
version/build to the complete nonignored source snapshot. Its typed local
schema requires the final release commit to descend from `prepared_from_sha`;
the typed CI schema requires exact `source_sha` equality. A clean archive embeds
its Git revision, source fingerprint, and source state. CI verifies those values
against `GITHUB_SHA`, and `scripts/export-ios-release.sh` refuses an archive that
does not match the clean checked-out release source.
Fingerprinting rejects tracked `assume-unchanged` and `skip-worktree` index
state so sparse or locally hidden files cannot masquerade as a complete clean
release checkout.
The embed phase also rejects traversal, a final plist symlink, or a
multiple-hard-link plist before writing provenance outside the canonical
product build directory.
`scripts/check-ios-project-resources.sh` additionally proves the preflight and
embed phases are defined once, invoke only the canonical scripts, belong only
to the main `Merian` target, and occupy the required first and final
product-mutating positions. Its portable detached, duplicate, wrong-command,
and phase-order fixtures live in
`scripts/test-check-ios-project-resources.sh`.
...
as a fallback. Its fixture test prevents expected negative-path application logs
from replacing the actual failed test and assertion in the job summary.
...
gain historical recovery authority from its earlier transaction-start
timestamp.
