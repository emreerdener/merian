...
V50 is frozen in `Models/Schema/SchemaV50Snapshots.swift`, and V51 owns the
active models plus its schema-scoped goal-hint companion. V47 through V50 remain
available in `SchemaVersions.swift` for source-specific startup recovery.
...
V51 resolves the SwiftData tombstone naming collision. The active
...
and acknowledgement-only purge contract remain unchanged. The frozen V50 graph
is used only as the source side of the lightweight V50 → V51 migration. The
canonical schema and recovery contract is in the
...
- V50 adds `OfflineQueuedScanGoalHint`, a scan-keyed companion containing the
  ...
  dedicated `[V49, V50, V51]` source-isolated plan, while stores stamped V50 use
  `[V50, V51]` and the tombstone rename stage. Older recent source-specific
  plans reach V49, advance through V50, and then apply V50→V51. The supported
  recent-source enum remains consecutive through the immediate predecessor of
  `CurrentSchema`; app dispatch is exhaustive and has no full-history fallback
  for an unhandled recent source. Disk fixtures run the production metadata
  decision before opening stores, and the source guardrail locks checksum retry
  order to current store followed by V50 through V42.
...
`SchemaV50Snapshots.swift`, while the active V51 declaration and migration plan
