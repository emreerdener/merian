...
| Data/Species Preferences | `apps/ios/Merian/Core/Data/SpeciesPreferences/` | Cross-feature SwiftData CRUD and lossless legacy promotion for preferred species display names; normalized timestamp-conflict policy with a consistent local-wins equality rule; exact PostgREST row/upsert values; the sole narrow live Supabase adapter with deterministic `(updated_at, scientific_name)` paging; and main-actor single-flight, trailing-request, clock-skew-safe freshness, account-lease, tombstone, and diagnostics coordination. The active SwiftData model remains in `Models/ActiveSchema`, and the legacy/pending-delete store remains in Core Preferences.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| Preferences              | `apps/ios/Merian/Core/Preferences/`             | `AppSettings` typed observable state; the small `UserDefaults`-backed Explore-share, field-note, and legacy species-name stores; the verified accepted-account-deletion inventory for account-derived caches; and an injected runtime-reset composer for observable settings, gamification, the generation-fenced app badge, and RAM images. Extracted owners import neither Supabase nor SwiftData. Device settings, consent, deletion-recovery state, and the Apple manual-revocation notice survive that account cleanup. Durable species reconciliation is owned by `Core/Data/SpeciesPreferences`; security recovery state and the exact key registry remain with their current staged owners.                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
