...
| Explore            | `apps/ios/Merian/Features/Explore/`                                                                                                    | Product-area-first public discovery feature. `Shell/` owns the root sheet/router, `Feed/` owns observations feed, post detail, comments, hashtags, and feed interaction state, `Map/` owns the map surface, `Identify/` owns Community ID requests/activity, `FieldTrips/` owns Field Trip Available/Recent tabs, guided template detail, progress, publication, and profile modules, `Notifications/` owns Explore activity notifications, `AuthorProfile/` owns public author sheets, `Shared/` holds only cross-area Explore UI helpers, and `Widgets/` writes the Explore widget cache.                                                                                                                                                                                           |
