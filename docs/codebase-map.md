...
| ------------------------ | ----------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| AI                       | `apps/ios/Merian/Core/AI/`                      | `InferenceEngine` observable orchestration, benchmark/media publication, and modality-specific effect sequencing; `Inference/Completion` accepted-result normalization, shared discovery/replacement/circuit/telemetry effects, committed biological events, exact-finalization authorization, and permit-gated notifications/milestones; `Inference/Request` injected visual/nonvisual request mapping, staged-video upload, and provider dispatch; `Inference/Result` injected live parse/save input normalization, typed persisted/no-record/rejected outcomes, tier confidence presentation bands, and synchronous persisted-replacement/metadata safety before repository deletion; `Inference/Recovery` stateless failure classification, modality-specific telemetry/circuit/feedback decisions, typed error presentation, and lookalike-cache reset policy; `Inference/Hydration` replaceable live/historical/identification-review task ownership, a value-only historical SwiftData projection, Auth draining, bounded request histories, persisted enrichment TTL, temporary backoff, structured Wikipedia/enrichment/GBIF child work, enrichment mapping behind an injected fetch seam, a sole live endpoint adapter, and immutable hydration persistence snapshots with live database/encoding effects; `Inference/State` contains exact live task/attempt identity behind an injected durable-queue core and sole live adapter, plus bounded write/review sequencing and Auth fencing; `Inference/LocalAnalysis` contains ephemeral model/cadence lifetime plus split Vision, bounded-image, deterministic-trait, phrase, cadence, and future Foundation-cue policies; generated Edge DTOs; the stateless shared response-preparation service; the foreground parse/save actor; and on-device viewfinder intelligence.                                                                                                                                                                                                                                                                                                                                                           |
...
`InferenceEngine.load(from:)` retains presentation replacement and releases the
previous live-media buffers before constructing that projection; the registered
historical task captures the projection rather than the managed
`LocalScanRecord`.

`Inference/State/InferenceLiveAttemptCoordinator.swift` is the concrete owner
for foreground task and exact process-local/durable attempt identity.
...
those collaborators once; the engine keeps observable publication and reviewed
...
two remaining production files above 600 lines: `InferenceEngine.swift` and
`SupabaseManager.swift`. Explore wire/request models now have focused owners
under `Core/Network/Models/Explore/`; shared semantic-location redaction lives
in `Core/Models/ExploreLocationPrivacy.swift`. The Codable post-location mode
lives in `Core/Network/Models/Explore/ExploreLocationSharingAPIModels.swift`,
while Explore-specific labels and symbols live in
...
enrichment adapter and keeps scheduling/application policy in `InferenceEngine`.
...
- `Core/Architecture/CoreIntegrationArchitectureTests.swift` freezes the Core
  root/domain and README inventories, the explicit two-file residual size
...
- `Core/AI/Inference/InferenceLiveQueueServiceTests.swift` owns exact durable
  ...
  replacement preservation, recovered-background admission, partial-identity
  rejection, and suspended same-scan finalization races.
