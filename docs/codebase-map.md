...
| Explore            | `apps/ios/Merian/Features/Explore/`                                                                                                    | Product-area-first public discovery feature. `Shell/` owns the root sheet/router, conversion from typed capture-goal destinations into optional focused Field Trip checklist-item routes, stack-based author-profile routing, the profile-to-scan nesting cap, and scoped video playback coordinator; `Feed/` owns observations feed, post detail, comments, hashtags, feed interaction state, the shared feed/detail media host, feed center Play/Pause versus outer navigation gesture policy, surface-aware video mute transitions, standalone-audio spectrogram playback, and device-local per-post audio boost preferences/DSP; `Map/` owns the map surface; `Identify/` owns Community ID requests/activity; `FieldTrips/` owns Field Trip Available/Community segments, Seasonal Challenges, guided template detail, focused target expansion/highlighting, its capture-goal provider mapping, progress, publication/challenge-entry detail, badges, and profile modules; `Notifications/` owns Explore activity notifications; `AuthorProfile/` owns public author profile content/routes; `Shared/` holds only cross-area Explore UI helpers; and `Widgets/` writes the image-only Explore widget cache. |
...
| Species Dictionary | `apps/ios/Merian/Features/SpeciesDictionary/`                                                                                          | Product-area-first species reference feature. `Detail/` owns the public species page, reference gallery, and authenticated Community sightings preview/paginated grid; `Catalog/` owns the Explore Index catalog/overview/regions surfaces; and `Tree/` owns the taxonomy canvas and graph model.                                                                                                                                                                                                                                                                                                                                                              |
...
| Media          | `apps/ios/Merian/Core/Media/`              | Shared bounded local-media processing, including adaptive audio boost and normalized spectrogram-seeking policy reused by Explore and private scan Insights without modifying source recordings.                                  |
...
- Field Trip capture-context decoding and provider mapping into generic goals,
  cross-outing order and wraparound, completion advancement, account-isolated
  caching, stale-data retention, exact artwork mapping, typed destinations, and
  focused Explore routing.
