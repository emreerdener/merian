...
On failure, `scripts/extract-ios-test-failure-diagnostics.sh` reads the
structured result summary first, then the failed test tree, and uses the raw
build log only as a fallback. Its fixture test prevents expected negative-path
application logs from replacing the actual failed test and assertion in the job
summary.
