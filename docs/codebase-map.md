...
purge integration stay with `AppDIContainerTests` and Core Security. The
cross-language `accountDeletionCoverage.test.ts`,
`purchasePrincipalMigrationContract.test.ts`, and
`ghostProfileMergeClientContract.test.ts` contracts read these extracted owners
directly; owner or suite rehomes must update their paths atomically. The
identity-free
