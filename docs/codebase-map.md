...
deterministic live-to-queue and queued-completion UI smokes, and independently
create an unsigned current-SHA Release archive without allocating a release
build. Distribution is owned solely by Xcode Organizer after the exact SHA
passes the full iOS workflow. Operators archive a clean `main` checkout with the
Merian scheme, choose **TestFlight & App Store**, and leave automatic signing
plus **Manage version and build number** enabled. Xcode and App Store Connect
own the unique uploaded build number; GitHub has no Apple signing or upload
