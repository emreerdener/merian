...
Within Core Hardware's camera boundary, the root `CameraManager.swift` retains
the AVFoundation stack, serial camera queue, delegate callbacks, lock-protected
live photo/video requests, and observable hardware state. `Camera/Models` owns
value-only recording identities, `Camera/Policies` owns deterministic microphone
reuse and recording-generation correlation, and `Camera/Coordination` owns the
MainActor target-FPS debounce task. `CameraArchitectureTests` prevents those
focused owners from reclaiming session, network, persistence, or UI effects and
holds the still-oversized live manager to an interim non-growth ceiling.
