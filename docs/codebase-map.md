...
| Insights           | `apps/ios/Merian/Features/Insights/`                                           | Product-area-first insight result feature. `Shell/` owns the root presentation, completed-scan routes, embedded navigation, root view model, idempotent persisted-completion precedence over stale same-ID queued routes, and generation-keyed result action/Field Notes activation; `Content/` owns biological/non-biological results, queued lifecycle UI, foreground analysis, direct-child-before-parent completion notification, and the shared `ScanningExperienceView` with intrinsic accessibility size around its animated status badge; `IdentificationReview/` owns candidates and confidence UX through platform-neutral Models, Services-only review/image/haptic/entitlement/persistence/routing effects, scan-and-generation-fenced ViewModels, composition Views, interaction-grouped Components, and narrow shared subject/database owners; `Media/` owns carousel/gallery, exact live-to-queue media continuity, export, and playback switching; `SpeciesReference/` owns shared observation charts, habitat/GBIF maps, taxonomy, lookalikes, and fallback reference images through platform-neutral Models, Services-only SwiftData/network/image/haptic/enrichment effects, generation-fenced ViewModels, a chart composition View, and domain-grouped Components; `FieldNotes/`, `Sharing/`, `Reporting/`, and `Toolbars/` own their named Insight subareas; `Toolbars/TopToolbar/` owns collection and audio-listening overflow actions plus the stable trailing queued-delete slot; `Shared/` holds reusable Insight chrome. Shared private conversation behavior is owned by the separate Field Chat feature.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
Within Insights Species Reference, platform-neutral `Models` own observation and
heatmap presentation policy; `Services` alone resolve the local SwiftData
projection, public observation endpoint, GBIF/Wikipedia sessions, bounded image
loading, haptics, and habitat enrichment; and generation-fenced `ViewModels`
publish observation, heatmap, and fallback-image state. `Views` owns the chart
...
`MerianTests/Features/Insights/SpeciesReference` enforce these boundaries,
checked feature sendability, removal of the former `Cards`/`Utilities` owners,
...
`CarouselSelectionCandidate` contract prevents the policy layer from depending
on SwiftUI-backed page values. `Builders` owns page assembly and transient
availability; `Services` is the only live audio-session, audio-boost, telemetry,
and haptic resolver; and `Playback` owns AVPlayer observation and cancellation
lifetimes. Root carousel views, `Pages`, and `Components` retain UI-only
selection, mute, focus, seek, animation, and mounted player state without
networking or direct singleton resolution. Mirrored tests under
`MerianTests/Features/Insights/Media` lock those boundaries and the 600-line
production-file ceiling. The image renderer shared by Insights, Explore, Field
Trips, and Species Dictionary is `Core/UI/Components/AsyncLocalImageView.swift`;
its live `LocalImageLoader` resolution belongs to
`Core/UI/Services/AsyncLocalImageDependencies.swift`. The domain-neutral native
pager, page identity value, zoom host, pagination dots, and top scroll-edge
treatment shared by Insight and Field Trips live in
`Core/UI/Components/MediaCarousel`; feature-owned page values project their own
stable controller-reuse keys into that boundary. Equal ID/reuse keys preserve a
mounted controller; a sequence or key change invalidates the native data-source
cache before the selected page is reinstalled.
...
| Media          | `apps/ios/Merian/Core/Media/`              | Shared bounded local-media processing, including `InferenceAudioPreparer` for secure historical-reference materialization and canonical queue/refinement WAV sidecars, adaptive audio boost, reusable spectrogram palette/raster/live-horizon policy, normalized seeking/display progress, immutable `SendableCGImage` concurrency transport, and `MediaPlaybackObservation`. The observation owner removes exact AVPlayer KVO/notification/periodic-time tokens and generation-fences late callbacks from replaced players.                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| UI             | `apps/ios/Merian/Core/UI/`                 | Shared controls, tab bar, media mode toggle, the presentation-only audio spectrogram shared by Capture and Insights, cross-feature scan-thumbnail projection/rendering with injected loader closures, generic empty-state presentation, domain-neutral goal progress ring, the presentation-only complimentary-scan display value shared by Settings and Insight, slide-to-confirm, reusable modifiers/theme, typed `ToastPayload`, and the bounded DI-owned milestone presenter/foreground-host system.                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `SpeciesObservationStatsDatabaseActor` | `apps/ios/Merian/Features/Insights/SpeciesReference/Services/SpeciesObservationStatsDatabaseActor.swift` | Ad-hoc behind `SpeciesObservationStatsDependencies` for each species chart load; filtered/projection SwiftData fetches local observation overlays, then delegates bucketing to `Features/Insights/SpeciesReference/Models/SpeciesObservationStatsReducer.swift`. |
