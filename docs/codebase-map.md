...
handling, TLS delegate, raw dispatch, and DEBUG override;
...
raw-response JSON POST bridges. Fixed-result Dictionary/stats bridges
...
`CoreNetworkIntegrationArchitectureTests.swift` freezes the exact 17
...
missing/empty/untrusted-chain rejection, injected-session dispatch, and exact
authenticated request construction. The cross-slice source guard keeps session
construction in `PinnedNetworkTransport`, per-attempt Auth leasing in
