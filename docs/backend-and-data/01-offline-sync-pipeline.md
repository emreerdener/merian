...
> **Inference Replay Deadlocks**: `runInferencePipeline` strictly executes inside a `defer { activeScanUploadIds.remove(scanId) }` guard wall. Because `replayInferenceForUploadedScans` manually locks the background queue ID array prior to execution to prevent execution overlap, transient network timeouts hitting the Gemini Edge node natively required this `defer` block. Without it, standard `catch` escapes from failed re-attempts permanently left the active lock trapped in memory, destroying future connection syncs forever until a hard kill.
