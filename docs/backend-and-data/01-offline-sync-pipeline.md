...
2. **`updateExistingScans`**: Fetches only local records whose IDs appear in the cloud response (predicate-scoped + `propertiesToFetch` projection). To avoid SQL IN-clause planner degradation on large libraries, `responseIds` is chunked into batches of 500 before each `#Predicate` is built. Updates: `localImagePath`, `additionalImagePaths`, `referenceImageUrl`, GPS fields, `locationName`, and taxonomic ranks (`taxonomyKingdom` through `taxonomyGenus`). Saves only if any field actually changed.
