...
**Startup reconciliation**: On first connectivity restore per process life, `replayInferenceForUploadedScans` runs `resetOrphanedInferencingScans()` (resets `.inferencing → .staged`) and `reconcileOrphanedUploadingScans(activeScanIds:)` (resets `.uploading → .pending` for scans with no active URLSession task). This is safe because no pipelines are running when a fresh process starts. Transient inference failures reset the scan back to `.staged` (via `transitionScan(id:to:)`) so `replayInferenceForUploadedScans` can reclaim it on the next connectivity restore.
