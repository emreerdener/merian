...
presentation-identity follow-up and evidence documentation, so its immutable
descendant SHA is pending\
...
This snapshot binds the runtime remediation to exact committed source
`a21155a3299598e81be0ec322ce339adbff62ff1`. Its parent
...
At review, `HEAD` and `origin/main` both resolved to `a21155a32` and the
worktree was clean. The portable gates below were then repeated against that
exact SHA. Subsequent call-site audit found that a failed SwiftData lookup for a
newly presented scan could retain the previous scan's record snapshot. The
current working tree adds exact engine/record/snapshot identity gating, stale
...
follow-up is not part of immutable SHA `a21155a32`; repeat all applicable gates
after it receives an immutable descendant SHA. Neither this source follow-up nor
the documentation converts local checks into hosted PostgreSQL, Xcode,
deployment, or TestFlight evidence.

Current-worktree portable repetition now passes parser acceptance for all 33
...
guardrails, DTO contracts, CI workflow contracts, and the critical-result
  validator with all 19 new exact cases. This mutable worktree evidence must
still be repeated from its immutable descendant SHA. Xcode compilation, full
tests, Release archive, fresh PostgreSQL catalogs, deployment, and TestFlight
proof remain hosted gates.
