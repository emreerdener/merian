...
on `main` as `cc664a20d6212299966b4579f733e612ed836514`. Current source also
repairs the exact-identity test fixtures exposed by the subsequent hosted run
without relaxing those runtime fences; retain the immutable descendant SHA from
the next complete hosted gate\
...
the three unique failures in the latest supplied hosted iOS run: repeated
...
At review, `HEAD` and `origin/main` both resolved to `cc664a20d`. The source
content later committed as that exact SHA passed the complete portable gate set,
and the focused exact-SHA parser, lint, and tooling gates were repeated after
commit. A call-site audit had found that a failed SwiftData lookup for a newly
presented scan could retain the previous scan's record snapshot. Commit
`cc664a20d` adds exact engine/record/snapshot identity gating, stale
...
joined follow-up and five menu/Field Notes regressions exposed by the newly
supplied 917-test hosted run. The missing-owner reconciliation case was already
protected. Xcode compilation, full tests, Release archive, fresh PostgreSQL
catalogs, deployment, and TestFlight proof remain hosted gates.

For the current fixture/gate follow-up, the Swift frontend parses both changed
...
protected case names resolve to maintained tests, and all 11 documentation
contracts pass. The large network test file retains its documented whole-file
baseline lint debt. These checks validate source and evidence tooling, not the
simulator execution still required below.
...
| Hosted iOS compile, unit, and archive gate | The newly supplied hosted run compiled and executed 917 tests in 67 suites, then reported 16 issues across six unique cases: missing-owner reconciliation retained a stale Explore post ID; two toolbar-policy fixtures could not confirm or reanalyze; and three Field Notes fixtures had no current scan identity. Source review proved that the production guards were behaving as designed. The missing-owner mock represented a handler-owned absence but omitted `X-Merian-Handler: 1`, while the Insight fixtures bypassed the production binding path and supplied only part of the engine/record/ID/snapshot topology. Current source corrects those fixtures and expands the critical-result gate from 53 to 58 exact cases without weakening route-availability or presentation-identity safety. | The supplied run remains failed evidence, and the prior 879-test failure remains historical evidence rather than acceptance. Only a full build, complete unit target, and Release archive that identify an immutable descendant containing `cc664a20d` plus the fixture/gate follow-up can close this gate. The rerun must report all six named cases as passed through the structured critical-result validator. |
