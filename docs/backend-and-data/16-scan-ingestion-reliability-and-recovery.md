...
The current worktree promotes the open child before parent refresh, makes a
persisted completion win every stale queued-route rebind, and keys result
toolbar/Field Notes tasks to the monotonic presentation generation. Production
queue timing, retry fences, cancellation behavior, and Supabase runtime behavior
are unchanged\
...
completed-record precedence are in the current worktree.
...
A local exact-case diagnostic run against the current worktree then proved the
transaction, direct promotion, and retained-media path: **Northern Cardinal**
replaced queued content in place, the decoded audio control remained available,
and native Back navigation remained intact. The next assertion exposed a
separate same-ID lifecycle defect: no bottom toolbar appeared, so neither
`FieldChatToolbarButton` nor `InsightShareButton` was reachable. The reveal task
had captured the queued presentation generation, was canceled by promotion, and
was keyed to `persistentScanId`; because the completed record intentionally
reuses that UUID, SwiftUI did not restart it. Field Notes synchronization had
the same task-identity risk.
...
expanding renderer. `ConfidenceBadge` now clips that glare to its
`GeometryReader`, excludes it from hit testing and accessibility, and clips the
translated text-reveal mask to the source text. `ScanningExperienceView` fixes
the composed badge to its intrinsic size, the smoke explicitly requires the
resulting Button frame to be contained by the app window, and the portable
workflow contract pins every protection.
...
strengthened toolbar assertions. That runtime result predates the
animation-bounds correction. The current extended source passes frontend parse
across all nine changed Swift files, strict focused SwiftLint with zero
violations, portable workflow contracts, and critical/focused result fixtures. A
subsequent sandbox policy denied CoreSimulator user-cache/device access and
SwiftPM manifest diagnostics, preventing another exact runtime execution here.
This is not reported as acceptance. A new hosted committed descendant must pass
all 1,243 unit tests, exactly one queued-scan UI smoke, and the current-SHA
archive together. PostgreSQL catalogs, deployment, TestFlight, and joined
staging proof remain hosted gates.
...
all 193 migration-source assertions across 30 discovered contract files, whole
tree formatting across 689 files, lint across 533 files, all 89
...
| Changed Swift source quality               | Retained broad evidence passed direct Debug simulator and optimized Release/device whole-module type checking across all 406 app sources, then type-checked all 84 unit-test and both UI-test sources with cached locked dependencies and macros enabled. A later incremental app/unit/UI `build-for-testing` succeeded, and the complete `InsightSheetViewModelTests` suite passed 89/89 with the same-ID route and toolbar fences present. That evidence predates the final scanning-badge animation-bounds correction. All nine Swift files in the current joined correction pass compiler-frontend parse and strict focused SwiftLint with zero violations. Fresh Xcode build attempts now stop before source compilation because the workspace sandbox denies CoreSimulator cache/device access and SwiftPM manifest diagnostic writes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Current syntax, changed-scope lint, and portable source contracts are verified. Prior build/runtime evidence is retained only for the source state it actually compiled; it is not misreported for the animation-bounds patch. A new hosted build remains required for current app/test cross-file references, actor isolation, macro expansion, linking, runtime tests, and archive acceptance.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| Hosted iOS compile, test, and archive gate | Run 103 on exact SHA `4f68e68913fca6276458cd093ad167c9bc7d5d9e` passed all 1,241 unit tests with zero failures or skips and passed the protected critical scan-flow validator. Its exact-SHA Release archive succeeded at 239,083,520 bytes for `1.0.2 (235)`, fingerprint `99c82c4e68eceb39c0d29db26bfe57236105de25c499dcd1a9acbe3c82e25c0e`, with verified main dSYM UUIDs and Debug-only UI-test seed markers absent. The exact queued-audio UI smoke failed after the explicit badge tap because the seeded completed record did not take over the queued sheet. One subsequent local diagnostic reached Northern Cardinal with decoded audio retained, then proved Field Chat and Share were still hidden by same-ID toolbar task identity. A later verbose rerun exposed the scanning badge's invalid 703-point off-window accessibility frame and XCTest's x=5 fallback tap.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Unit and archive evidence are retained green only for exact SHA `4f68e68913`; focused UI acceptance is red. The current worktree orders child promotion before parent refresh, prefers persisted completion over stale same-ID queued routes, keys result toolbar/notes tasks to the presentation generation, constrains the animated badge to intrinsic bounds, and rejects an off-window trigger frame. Its two added exact cases make the expected complete unit count 1,243. A new hosted committed descendant must compile current source and pass all 1,243 unit tests, exactly one queued-scan UI case, and its current-SHA archive together before iOS production readiness is green.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| Production deployment and observation      | Runs 1549–1552, 1569, `1a75179dd`, and the latest 26-file catalog run all stopped before production mutation; no production evidence has yet been supplied for joined committed source `4f68e68913` or a committed descendant                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Pending exact-SHA catalog acceptance, ordered migration/function deployment, matching iOS release, and a clean observation window                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
