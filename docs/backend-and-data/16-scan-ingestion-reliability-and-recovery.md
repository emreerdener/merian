...
after completion, preventing filename-only media evidence. These descendants
passed the portable gate set described below, but the focused UI runtime still
requires one fresh immutable-SHA hosted result. A call-site audit
...
292 runtime files; 191 migration contracts; 110 deployment-tooling and 12
...
validator with 61 protected exact cases total, including all 19 added by the
...
network-boundary media-incident compatibility case.
The missing-owner reconciliation case was already protected.
A separately focused frozen Field Chat run passed 30 tests with 0 failures
...
prompt safety, prompt suggestions, and atomic reservation source contracts.
...
| iOS portable release tooling               | Scope, workflow, structured failure extraction, complete-unit critical-result, and focused-UI contracts passed. The unit validator rejects absent or skipped exact regressions across analysis, replay, deletion, Explore, Community, and Field Chat. The focused validator independently rejects empty, malformed, failed, skipped, wrong-suite, wrong-case, and duplicate evidence for `testQueuedAudioScanRetainsAudioAcrossCompletionHandoff`. | Verified locally without invoking a simulator; this validates the fail-closed evidence parsers and workflow wiring, not execution of either protected test run on Xcode. |
