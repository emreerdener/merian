...
validator with 53 protected exact cases total, including all 19 added by the
joined follow-up. Xcode compilation, full tests, Release archive, fresh
PostgreSQL catalogs, deployment, and TestFlight proof remain hosted gates.
...
| Hosted iOS compile, unit, and archive gate | The latest supplied hosted run compiled and executed 879 tests in 67 suites, then failed with three unique regressions: `completedInferenceAndQueueDeletionCommitTogether()` rejected `repeatedDelete`; malformed Explore share and share-state responses leaked `DecodingError` instead of `MerianError.invalidResponse`. Exact committed joined source `cc664a20d` addresses all three, includes recovery-proof hardening and replay process-single-flight, and adds direct negative invariants plus broader malformed-response coverage.                                                                                                                                                                                                                                                                         | The supplied run remains failed evidence. Only a full build, complete unit target, and Release archive that identify `cc664a20d` or a documented descendant containing the same runtime source can close this gate                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
