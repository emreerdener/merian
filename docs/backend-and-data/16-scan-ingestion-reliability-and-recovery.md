...
At this review, `HEAD` and `origin/main` both resolved to `bdf84b52`. The joined
...
evidence parser. The descendants passed the complete portable gate set plus the
focused exact-SHA parser and lint checks described below. A call-site audit had
found that a failed SwiftData lookup for a newly presented scan could retain the
previous scan's record snapshot. Commit `cc664a20d` adds exact
