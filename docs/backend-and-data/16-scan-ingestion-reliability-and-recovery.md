...
to pass, and require the deterministic live-Insight-to-queue and queued-scan
completion UI smokes to report their exact two-case passed set; a scope-only
success is not release evidence, and neither is UI-bundle compilation without
execution. Old clients are compatible with the new backend; the new client must
not be used to compensate for an old false-success function.
...
- the deterministic live-Insight-to-queue and queued-scan-completion UI smokes,
  followed by the exact-SHA Release archive.
...
complete unit target, the exact two-case live-to-queued/completion UI smoke set,
and the current-SHA archive together. PostgreSQL catalogs, deployment,
TestFlight, and joined staging proof remain hosted gates.
