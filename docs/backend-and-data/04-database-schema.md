...
**File layout:** Each schema version lives in its own file (`merian/Models/Schema/SchemaV1.swift` through `SchemaV17.swift`). The sole remaining purpose of `merian/Models/SchemaVersions.swift` is to declare `MerianMigrationPlan` — the ordered list of schemas and migration stages. When bumping to a new version, add a `SchemaV{N+1}.swift` file, append it to `MerianMigrationPlan.schemas`, add the migration stage (lightweight or custom), and update `Aliases.swift`. No other files need to change. The current active schema is `MerianSchemaV17`.
