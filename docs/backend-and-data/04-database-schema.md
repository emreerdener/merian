...
- `descriptions` (JSONB): Locale-keyed text payloads, e.g. `{"en": {"wikipedia": "..."}}`.
...
- `diagnostic_primary_rationale` (Text): The primary identification rationale generated by `gemini-2.5-flash` for the first low-confidence scan of this species (confidence < 0.85). Cached at the species level — subsequent low-confidence scans for the same species read from here instead of calling Gemini again. Added in migration `20260326200000_add_diagnostic_comparison.sql`.
...
- `insightDescription`: String — stores the Gemini vision model's per-scan identification reasoning (the `insight_data.ai_reasoning` value from the `/identify` response). Unique to each photo submitted.
...
- `habitatDescription`: String? (Added in `MerianSchemaV15`. Populated asynchronously by `BackgroundDatabaseActor.updateScanWithEnrichment` after `enrich-scan` returns for Pro users. For Pro users this loads 2–3 seconds after the scan completes via `InferenceEngine.fetchAndApplyEnrichment`. Free users see the `PremiumInsightsCard` paywall; on purchase, `triggerEnrichment()` calls `InferenceEngine.fetchAndApplyEnrichment` which hits the same `/enrich-scan` endpoint — but the endpoint enforces a Pro tier gate and returns `403` until the entitlement is active.)
- `globalDistributionRegionsJson`: String? (Added in `MerianSchemaV15`. Stored as a JSON-encoded string array (e.g. `["US-TX","MX"]`) due to SwiftData's lack of native typed array codable support. Decoded to `[String]` in `InferenceEngine.fetchAndApplyEnrichment` for `SpeciesData.globalDistributionRegions`. Populated asynchronously alongside `habitatDescription` via `BackgroundDatabaseActor.updateScanWithEnrichment`.)
