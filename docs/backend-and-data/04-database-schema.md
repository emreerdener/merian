...
- `public.get_explore_feed(self_id UUID, max_limit INTEGER, before_shared_at TIMESTAMPTZ, before_post_id UUID)`: Primary shipped feed projection. It returns reverse-chronological feed rows with public author identity, hero image URL, coarse location, optional public telemetry (`time_of_day`, `current_month`, `weather_condition`, `weather_temperature_f`), denormalized like/comment counts, and viewer-specific flags (`viewer_has_liked`, `is_owned_by_viewer`). `author_avatar_url` is sourced from `public.users.public_avatar_url`, never directly from `auth.users`. Paging is stable on `(shared_at DESC, post_id DESC)` so new posts inserted above the viewer do not cause skips or duplicates while scrolling.
...
The current active schema is `MerianSchemaV41`. Recent milestones:
...
- V41 added first-class `CapturedMediaEntry` rows and `capturedMediaEntries` relationships on both `LocalScanRecord` and `OfflineQueuedScan`, while retaining `capturedMediaJSON` as a compatibility mirror.
