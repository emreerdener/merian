...
- `isFlagged: Bool` — mirrors `LocalScanRecord.isFlagged`. When `true`, overrides the `ConfidenceBadge` state natively to display an "Under Review" status lock and prevents interaction with `CandidatesCard` by showing a sticky `FlaggedView` overlay.
