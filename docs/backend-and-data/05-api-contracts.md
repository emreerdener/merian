...
`gbif_taxon_key` is `null` when the species has not yet been matched by GBIF (Cache Miss species where `identify`'s background task has not yet completed). `diagnostic_comparison` is `null` when the scan's `ai_confidence_score ≥ 0.85`. The iOS client (`InferenceEngine.fetchAndApplyEnrichment`) applies the same 0.85 threshold to write `speciesData.diagnosticComparison` but the display of the UI itself is further gated dynamically per tier (e.g. 0.88 for flash, 0.80 for pro) in `BiologicalView`.
