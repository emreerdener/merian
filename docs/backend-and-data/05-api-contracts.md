...
An enrichment endpoint that asynchronously surfaces habitat, distribution, and (when confidence is low) diagnostic comparison data for a scan. Called automatically by the iOS client after every successful biological scan completes — the user sees a loading skeleton in `SpeciesInsightsCard` while this request is in flight.
