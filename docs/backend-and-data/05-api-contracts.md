...
**`common_name` source**: For biological subjects, `common_name` is never generated by the Gemini vision call. It is always spliced from `species_dictionary.common_names["en"]` (Cache Hit) or from a dedicated `fetchStaticEncyclopedicData` text inference call (Cache Miss). The Swift decoding layer applies `.capitalized` on rendering for display consistency.
...
A DB-first enrichment endpoint that surfaces premium habitat and distribution data for free users. Gemini is only called as a fallback when the data does not already exist.
...
- **Verification**: Evaluates `scan_id` ownership using the `user.id` resolved from the GoTrue JWT against the `scans` table. Returns `403 Forbidden` if missing or malicious.
- **DB-First Cache Check**: Queries `species_dictionary` for existing `habitat_description` and `global_distribution_regions`. If both are present, returns immediately — no Gemini call, no cost. Because the `/identify` Cache Miss path writes this data for every species regardless of tier, most `/enrich-scan` calls are satisfied from the DB.
- **Gemini Fallback**: Only fires when the species has never been enriched. Uses Gemini 2.5 Flash (text-only, no image payload) with `maxOutputTokens: 1500`. Writes results to `species_dictionary` so all future callers — free or Pro — get a cache hit.
- Returns the enriched object under `data`. Does not write to the `scans` table — `ai_reasoning` for a scan is set at original scan time and does not change on ad-hoc enrichment.
