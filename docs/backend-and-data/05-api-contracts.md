...
```json
{
  ...
  "// Cache Hit + Pro tier only — sourced from species_dictionary:": "",
  "premium_insights": {
...
A Pro-gated enrichment endpoint that asynchronously surfaces habitat, distribution, and (when confidence is low) diagnostic comparison data for a scan. Called automatically by the iOS client after a successful Pro scan completes — the user sees a loading skeleton in `PremiumInsightsCard` while this request is in flight.
...
**Pro Tier Gate**: Returns `403 Forbidden` immediately if the authenticated user is not `subscription_tier = 'pro'`. Free users' enrichment data is still generated by the `identify` background task and cached in `species_dictionary`, but this endpoint enforces the paywall on retrieval so free users cannot access it without upgrading.

**Ownership Verification**: Validates `scan_id` against the `scans` table using the GoTrue JWT user identity. Returns `403 Forbidden` if missing or mismatched.

**`DIAGNOSTIC_THRESHOLD = 0.85`**: A scan's `ai_confidence_score` is compared against this constant. If below threshold, diagnostic comparison data is also fetched/generated. The diagnostic is a species-level cache — if the data already exists in `species_dictionary` from a prior low-confidence scan of the same species, it is returned immediately without a Gemini call.

**Full Cache Hit**: If `species_dictionary` already has `habitat_description`, `global_distribution_regions`, and (when needed) `diagnostic_primary_rationale`, the function returns all data immediately with no Gemini calls — typically sub-50ms.
...
`diagnostic_comparison` is `null` when the scan's `ai_confidence_score ≥ 0.85`. The iOS client (`InferenceEngine.fetchAndApplyEnrichment`) applies the same threshold client-side before writing `speciesData.diagnosticComparison`, ensuring the diagnostic UI is never shown for high-confidence scans even if the field is somehow non-null.
...
| `403` | `{ "error": "Forbidden: Premium insights require a Pro subscription." }` | User is not Pro |
| `403` | `{ "error": "Forbidden: Scan not found or does not belong to the user." }` | IDOR or scan not found |
