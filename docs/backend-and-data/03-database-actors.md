...
- `markScanAsStaged(scanId:r2Keys:)` — called once the last media upload for a
  ...
  `stagedR2Keys` and transitions `.uploading → .staged`. Source-state guard:
  only advances from `.uploading`; prevents a concurrent tombstone from being
  resurrected. Save failure rolls back the actor context.
