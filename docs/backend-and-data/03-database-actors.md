...
- `processAndCleanupOfflineScan(...)` — decodes an edge inference result, inserts a `LocalScanRecord` (including `candidatesData`, `inferenceTier`, and `imageQualityScore` from the edge response), and deletes the `OfflineQueuedScan` in a **single atomic `modelContext.save()`**. Uses explicit fetch-then-delete (not `delete(model:where:)`) to ensure MainActor `@Query` views receive change notifications. Returns an `OfflineScanProcessingResult` with four fields: `resolvedSpeciesName: String?`, `isNewDiscovery: Bool`, `finalScanId: String?`, and `speciesData: SpeciesData?` — the last field is non-nil when inference succeeded (i.e. `confidenceScore > 0`) and is used by `processInferenceDownloadResult` to hydrate a suspended `InferenceEngine` directly. On save failure, `resolvedSpeciesName`, `finalScanId`, and `speciesData` are all cleared so the caller does not fire push notifications, record discoveries, or hydrate the engine for an uncommitted scan.
