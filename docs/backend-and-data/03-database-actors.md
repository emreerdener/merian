...
- `processAndCleanupOfflineScan(...)` — the top-level orchestration boundary. Decodes an edge inference result and bridges three distinct inner operational layers inside a **single atomic context frame**:
  ...
  2. `insertLocalScanRecordIfMissing()`: Builds and persists the final `LocalScanRecord` (including `candidatesData`, `inferenceTier`, and `imageQualityScore`).
  3. `removeQueuedScanAndPersist()`: Cleans up the `OfflineQueuedScan` entry securely with a bounded fallback lookup. Uses explicit fetch-then-delete (not `delete(model:where:)`) to ensure MainActor `@Query` views receive change notifications. On overarching save failure, `resolvedSpeciesName`, `finalScanId`, and `speciesData` are all cleanly rolled back so the caller avoids emitting ghost notifications or hydrating an engine that lacks a formal database UUID payload.
