...
| Client proof durability       | `ghostProfileMergeClientContract.test.ts` directly reads `SupabaseManager`, the Ghost store, policy, workflow, policy test, and endpoint-adapter test. It also reads the Consent facade, runtime, cloud-session core/live adapter, state projection, synchronization, restoration, Realtime, repository, retry, merge, and focused tests. Together those inputs pin persistence before session replacement; device-only verified storage; restored-session retry; account-work lease/session adoption; cancellation, generation, and stale-account fences; provider and local-evidence completion before proof removal; verified consent persistence before publication; pending-evidence flush before remote refetch; owner-filtered Realtime and complete synchronization/restoration/removal draining; canceled-retry rejection after attempt reuse; terminal-only proof deletion; and authoritative merge before analytics application. Any direct source or test rehome must update the contract path atomically. Retryable 503 proofs remain queued. |
