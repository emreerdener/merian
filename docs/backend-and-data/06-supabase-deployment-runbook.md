...
| Client proof durability       | `ghostProfileMergeClientContract.test.ts` reads the extracted `GhostProfileMergeStore`, `GhostProfileMergePolicy`, and `GhostProfileMergeWorkflow` owners. It pins persistence before the session switch, `WhenUnlockedThisDeviceOnly` storage with read-after-write, restored-session retry before identity refresh, cancellation fences around every asynchronous finalization phase, proof removal after provider and local-evidence synchronization, terminal-only deletion, and target-owned pending-consent flush before account refetch. The consent-order assertion reads `MerianTests/Core/Security/Consent/ConsentManagerAuthorityTests.swift`; any owner or test rehome must update its contract path atomically. Retryable 503 proofs must remain queued. |
