...
| Client proof durability       | `ghostProfileMergeClientContract.test.ts` directly reads `SupabaseManager`, `GhostProfileMergeStore`, `GhostProfileMergePolicy`, `GhostProfileMergeWorkflow`, `GhostProfileMergePolicyTests`, and `GhostProfileMergeEndpointErrorAdapterTests`. It pins persistence before the session switch, `WhenUnlockedThisDeviceOnly` storage with read-after-write, restored-session retry before identity refresh, cancellation fences around every asynchronous finalization phase, proof removal after provider and local-evidence synchronization, and terminal-only deletion. Its target-consent assertion also reads `ConsentManager`, `ConsentSynchronizationCoordinator`, `ConsentSynchronizationMergePolicy`, `ConsentRealtimeCoordinator`, `ConsentRealtimeCoordinator+Live`, `RequiredConsentRestorationCoordinator`, `ConsentLedgerRepository`, `ConsentRetryPolicy`, `ConsentManagerAuthorityTests`, `ConsentSynchronizationCoordinatorTests`, `ConsentRealtimeCoordinatorTests`, and `ConsentRestorationCoordinatorTests` to require a current synchronization context, owner-filtered Realtime construction and generation/retry fencing, retention and drain of every outstanding synchronization handle—including superseded and previously invalidated work—UUID-keyed retention and Auth-transition drain of restoration retries, canceled-retry admission after manual retry reuses an attempt number, stale-account rejection, verified consent persistence before local publication, pending-evidence flush before remote refetch, and authoritative merge before analytics application. Any direct source or test owner rehome must update its contract path atomically. Retryable 503 proofs must remain queued. |
