...
| Client proof durability       | `ghostProfileMergeClientContract.test.ts` pins persistence before the session switch, `WhenUnlockedThisDeviceOnly` storage with read-after-write, restored-session retry before identity refresh, terminal-only deletion, and target-owned pending-consent flush before account refetch. That consent-order assertion reads `MerianTests/Core/Security/Consent/ConsentManagerAuthorityTests.swift`; a test rehome must update the contract path atomically. The Swift discard-policy source contract requires retryable 503 proofs to remain queued. |
