...
**Partial-failure handling**: If `applyUserTombstone` or `queueStorageDeletion` throws after auth has already been successfully revoked, the function logs a structured error via `logStructuredError` with `event: "safe_delete_partial_failure"` and `action_required: "Manually run apply_user_tombstone RPC"`, then re-throws the error so the response is `500` rather than a false-success `200`. This prevents silent data-integrity gaps where auth is gone but the tombstone or storage queue was never applied.
...
**Per-user daily rate limit (`enrich-scan/index.ts`)**: Free-tier users are throttled after 50 `enrich-scan` requests per day as a proxy for LLM budget control. Pro users are exempt. The check calls `getTierForUser` from `_shared/tierCache.ts` and returns `429 Too Many Requests` before any Gemini call when the quota is exceeded.
