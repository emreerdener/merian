...
9. **Species Dictionary Upsert**: Calls `supabaseAdmin.from('species_dictionary').upsert()` with `{ onConflict: "scientific_name", ignoreDuplicates: false }`. This always merges on conflict rather than skipping, ensuring that locale-miss Cache Misses can add new `common_names` entries to an existing row. To prevent lower-quality Flash-generated data from overwriting previously stored Pro-sourced taxonomy, toxicity, IUCN status, habitat, and distribution, all those fields are written using `??` null-coalescing — existing non-null values are always preserved. Only `common_names` and `descriptions` are unconditionally merged, as they are intentionally keyed dictionaries.
10. **Tier Resolution + Ghost Upsert (split critical path / background)**: Tier resolution is split: a lightweight `SELECT subscription_tier` runs on the critical path (before the Gemini call) to choose the model, served from `_tierCache` on cache hit; the ghost-user upsert (`{ onConflict: "id", ignoreDuplicates: true }`) stays in the background task and only runs if the SELECT returned no record. A module-scope `_tierCache` (`Map<userId, { tier, ts }>` with a 5-minute TTL) persists across warm Deno isolate re-use — cache hits on the critical path are near-instant and the background task always gets a cache hit after the critical-path SELECT. `ignoreDuplicates: true` ensures an existing user's `subscription_tier` is never overwritten by the ghost-user path.
11. **Scan Insert**: Calls `supabaseAdmin.from('scans').insert()` using the service role key, binding the scan to the authenticated `user.id`. All environmental telemetry (time of day, month, locale, semantic location, LiDAR depth scale), Google Cloud LLM token metrics (`llm_prompt_tokens`, `llm_candidate_tokens`, `llm_total_tokens` from Gemini's `usageMetadata`), `ai_reasoning` (Gemini's 2–4 sentence visual identification justification), `colors` (visual descriptors), and `group_tags` (2–4 plain-English categorical labels such as `["bird", "songbird"]`) are all written in the same insert. `colors` and `group_tags` feed into `semanticTags` on the Swift side for full-text search. `ai_reasoning` is fetched back during historical cloud sync and stored as `LocalScanRecord.aiReasoning`.
