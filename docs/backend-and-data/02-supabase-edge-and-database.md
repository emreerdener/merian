...
6. **Moderation Pipeline (`moderation.ts`)**: Evaluates Gemini Safety Ratings before any write occurs. Unsafe media sets the user's status to `SHADOWBANNED`, increments abuse strikes, and deletes the R2 object. Safe media falls through to the Rolling Cloud Window storage policy, which selects the appropriate Cloudflare R2 lifecycle bucket based on `.userTier`. `moderation.ts` calls `getR2Config()` once at the top and reuses the resulting `AwsClient` — it no longer calls both `getS3Client()` and `getR2Config()` separately, which previously created two `AwsClient` instances. **Moderation ERROR guard**: When `modResult.status === "ERROR"` (e.g. abuse strike DB write failed), background ingestion halts immediately. The scan is not inserted — without this guard, a moderation pipeline failure would create a permanent DB record with `image_storage_urls: null`.
...
The `/identify-multimodal` Edge Function replaces the legacy single-modality endpoints (`/identify`, `/audio-spec`, `/identify-sighting`) with a unified pipeline capable of ingesting any array combination of images, audio, and textual descriptions.

1. **Payload Assembly**: It accepts `imageBase64s`, `audioBase64s`, and `observation_contexts` arrays.
2. **WAV Preprocessing**: Audio data is preprocessed concurrently via `processWAV` (mono mix, silence trim, 16kHz resample) to optimize token payload size and reduce Gemini processing latency.
...
4. All database logic operates on these multi-modal arrays, persisting results to `scans.audio_r2_keys` (via V39 schema) and mapping `observationContextsJSON` within `LocalScanRecord`. Legacy endpoints are maintained temporarily to allow older iOS app versions to safely phase out.
